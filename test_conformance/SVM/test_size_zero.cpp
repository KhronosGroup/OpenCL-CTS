                                ...
                                cl_svm_mem_flags svmFlags)
{
    ...
    log_info("      clEnqueueSVMMemFill with zero size returned %d\n", error);
    ...
    log_info("      clEnqueueSVMMemcpy to SVM with zero size returned %d\n",
             error);
    ...
    log_info("      clEnqueueSVMMemcpy from SVM with zero size returned %d\n",
             error);
    ...
    log_info("      clEnqueueSVMMigrateMem with zero size returned %d\n",
             error);
...
REGISTER_TEST(svm_size_zero)
{
    ...
    if (svmCaps & CL_DEVICE_SVM_COARSE_GRAIN_BUFFER)
    {
        ...
        result |= svm_size_zero_helper(device, context, queue, 0);
    }
    if (svmCaps & CL_DEVICE_SVM_FINE_GRAIN_BUFFER)
    {
        log_info("    testing fine-grain SVM\n");
        result |= svm_size_zero_helper(device, context, queue,
