...
REGISTER_TEST(negative_create_context)
{
    ...
    test_object_failure_ret(
        ...
        "clCreateContext should return CL_INVALID_PLATFORM when: \"an invalid "
        "platform object is used with the CL_CONTEXT_PLATFORM property\" using "
        "a valid object that is NOT a platform",
        TEST_FAIL);
    ...
    ctx = clCreateContext(nullptr, 1, reinterpret_cast<cl_device_id*>(&context),
                          nullptr, nullptr, &err);
    test_object_failure_ret(
        ctx, err, CL_INVALID_DEVICE,
        "clCreateContext should return CL_INVALID_DEVICE when: \"any device in "
        "devices is not a valid device\" using a pointer to a valid object "
...
REGISTER_TEST(negative_create_context_from_type)
{
    ...
    props[1] = reinterpret_cast<cl_context_properties>(device);
...
REGISTER_TEST(negative_retain_context)
{
    ...
    err = clRetainContext(reinterpret_cast<cl_context>(device));
    test_failure_error_ret(err, CL_INVALID_CONTEXT,
                           "clRetainContext should return CL_INVALID_CONTEXT "
                           "when: \"context is not a valid OpenCL context\" "
                           "using  a valid object which is NOT a context",
                           TEST_FAIL);
...
REGISTER_TEST(negative_release_context)
{
    ...
    err = clReleaseContext(reinterpret_cast<cl_context>(device));
    test_failure_error_ret(err, CL_INVALID_CONTEXT,
                           "clReleaseContext should return CL_INVALID_CONTEXT "
                           "when: \"context is not a valid OpenCL context\" "
                           "using a valid object which is NOT a context",
                           TEST_FAIL);
...
REGISTER_TEST(negative_get_context_info)
{
    ...
    err = clGetContextInfo(reinterpret_cast<cl_context>(device),
                           CL_CONTEXT_REFERENCE_COUNT, sizeof(param_value),
                           &param_value, nullptr);
    test_failure_error_ret(
        err, CL_INVALID_CONTEXT,
        "clGetContextInfo should return CL_INVALID_CONTEXT when: \"context is "
        "not a valid context\" using a valid object which is NOT a context",
        TEST_FAIL);
...
REGISTER_TEST_VERSION(negative_set_context_destructor_callback, Version(3, 0))
{
    ...
    err = clSetContextDestructorCallback(reinterpret_cast<cl_context>(device),
                                         callback, nullptr);
    test_failure_error_ret(
        err, CL_INVALID_CONTEXT,
        "clSetContextDestructorCallback should return CL_INVALID_CONTEXT when: "
