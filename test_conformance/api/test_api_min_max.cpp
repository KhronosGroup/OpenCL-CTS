...
REGISTER_TEST(min_max_parameter_size)
{
    ...
    while (numberOfIntParametersToTry > 0)
    {
        ...
        {
            log_info(
                "Trying a kernel with %ld int arguments (%llu bytes) and one "
                "cl_mem (%llu bytes) for %zu bytes total.\n",
        ...
        {
            log_info(
                "Trying a kernel with %ld long arguments (%llu bytes) and one "
                "cl_mem (%llu bytes) for %llu bytes total.\n",
                numberOfIntParametersToTry,
        ...
        if (gHasLong)
        {
            ...
            {
                log_info("Results verified at %llu bytes of arguments.\n",
                         sizeof(cl_mem)
