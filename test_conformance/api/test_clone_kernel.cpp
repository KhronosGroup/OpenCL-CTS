...
int test_cloned_kernel_args(cl_device_id device, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    cl_int error;
    clProgramWrapper program;
...
int test_buff_image_multiple_args(cl_device_id device, cl_context context,
                                  cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
                               ...
                               cl_command_queue queue, int num_elements)
{
    ...
    if (svmCaps != 0)
    {
        ...
        cl_int* svmPtr_cloneKernel_2 =
            (cl_int*)clSVMAlloc(context, CL_MEM_READ_WRITE, sizeof(cl_int), 0);
        SVMCleanup<4> cleanup{ context,
                               { svmPtr_srcKernel, svmPtr_srcKernel_1,
                                 svmPtr_cloneKernel_1, svmPtr_cloneKernel_2 } };
        test_assert_error(
...
REGISTER_TEST_VERSION(clone_kernel, Version(2, 1))
{
    ...
    if (test_cloned_kernel_args(device, context, queue, num_elements)
        == TEST_FAIL)
    {
        test_fail("clCloneKernel test_cloned_kernel_args failed.\n");
    }

    if (test_cloned_kernel_exec_info(device, context, queue, num_elements)
        == TEST_FAIL)
    {
