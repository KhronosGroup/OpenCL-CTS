...
REGISTER_TEST(negative_invalid_arg_mem_obj)
{
    ...
    // Run the test - CL_INVALID_MEM_OBJECT
    error = clSetKernelArg(mem_obj_arg_kernel, 0, sizeof(buffer), nullptr);
    test_failure_error_ret(
        error, CL_INVALID_MEM_OBJECT,
        "clSetKernelArg is supposed to fail with CL_INVALID_MEM_OBJECT when "
        "argument is declared to be a memory object and the specified "
