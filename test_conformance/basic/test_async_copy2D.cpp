...
int test_copy2D(cl_device_id deviceID, cl_context context,
  ...
  size_t localStorageSpacePerWorkitem =
      lineCopiesPerWorkItem * elementSize *
  ...
  size_t inBufferSize = elementSize * (totalLines * numElementsPerLine +
                                       (totalLines - 1) * srcStride);
  size_t outBufferSize = elementSize * (totalLines * numElementsPerLine +
                                        (totalLines - 1) * dstStride);
  ...
  log_info("Global: %d, local %d, local buffer %db, global in buffer %db, "
           ...
           lineCopiesPerWorkgroup, lineCopiesPerWorkItemInt);
  ...
  for (int i = 0;
    ...
    if (i <
        (int)(globalWorkgroupSize * lineCopiesPerWorkItem - 1) * elementSize) {
      ...
      if (memcmp(((char *)outBuffer) + outIdx, ((char *)outBufferCopy) + outIdx,
                 dstStride * elementSize) != 0) {
        if (failuresPrinted == 0) {
          // Print first failure message
          log_error("ERROR: Results of copy did not validate!\n");
        }
        log_error("2D copy corrupted data in output buffer in the stride "
                  "offset of line %d\n",
                  i);
        failuresPrinted++;
      }
