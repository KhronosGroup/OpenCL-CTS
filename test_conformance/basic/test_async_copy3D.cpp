...
"   for(j=0; j<numLines; j++)\n"
"     for(k=0; k<numElementsPerLine; k++)\n"
"       localBuffer[ (get_local_id( 0 )*planesCopiesPerWorkItem+i)*(numLines*numElementsPerLine + (numLines - 1)*srcLineStride + srcPlaneStride) + j*(numElementsPerLine + srcLineStride) + k ] = (%s)(%s)0;\n"
// Do this to verify all kernels are done zeroing the local buffer before we try the copy
"    barrier( CLK_LOCAL_MEM_FENCE );\n"
" for(i=0; i<planesCopiesPerWorkItem; i++)\n"
"   for(j=0; j<numLines; j++)\n"
...
"    barrier( CLK_LOCAL_MEM_FENCE );\n"
"    event_t event;\n"
"    event = async_work_group_copy_3D3D((__global %s*)(dst+planesCopiesPerWorkgroup*get_group_id(0)*(numLines*numElementsPerLine + (numLines - 1)*dstLineStride + dstPlaneStride)), (__local const %s*)localBuffer, (size_t)numElementsPerLine, (size_t)numLines, srcLineStride, dstLineStride, planesCopiesPerWorkgroup, srcPlaneStride, dstPlaneStride, 0 );\n"
"    wait_group_events( 1, &event );\n"
"}\n" ;
                ...
                )
{
    ...
    size_t localStorageSpacePerWorkitem = elementSize*(planesCopiesPerWorkItem*(numLines*numElementsPerLine + (numLines - 1)*(localIsDst ? dstLineStride : srcLineStride) + (localIsDst ? dstPlaneStride : srcPlaneStride)));
    ...
    size_t inBufferSize = elementSize*(totalPlanes*(numLines*numElementsPerLine + (numLines - 1)*srcLineStride) + (totalPlanes - 1)*srcPlaneStride);
    size_t outBufferSize = elementSize*(totalPlanes*(numLines*numElementsPerLine + (numLines - 1)*dstLineStride) + (totalPlanes - 1)*dstPlaneStride);
    ...
    for(int i=0; i<(int)globalWorkgroupSize*planesCopiesPerWorkItem*elementSize; i+=elementSize)
    {
        for(int j=0; j<(int)numLines*elementSize; j+=elementSize)
        {
            ...
            {
                int inIdx = i*(numLines*numElementsPerLine + (numLines - 1)*srcLineStride + srcPlaneStride) + j*(numElementsPerLine + srcLineStride) + k;
                int outIdx = i*(numLines*numElementsPerLine + (numLines - 1)*dstLineStride + dstPlaneStride) + j*(numElementsPerLine + dstLineStride) + k;
            ...
            if (j < (int)(numLines - 1)*elementSize)
            {
                int outIdx = i*(numLines*numElementsPerLine + (numLines - 1)*dstLineStride + dstPlaneStride) + j*(numElementsPerLine + dstLineStride) + numElementsPerLine*elementSize;
        ...
        {
            int outIdx = i*(numLines*numElementsPerLine + (numLines - 1)*dstLineStride + dstPlaneStride) + (numLines*elementSize)*(numElementsPerLine) + ((numLines-1)*elementSize)*(dstLineStride);
