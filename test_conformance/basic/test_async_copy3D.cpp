...
int test_copy3D(cl_device_id deviceID, cl_context context,
  ...
  size_t localStorageSpacePerWorkitem =
      elementSize * (planesCopiesPerWorkItem *
  ...
  size_t maxTotalPlanesIn =
      ((max_alloc_size / elementSize) + srcPlaneStride) /
      ((numLines * numElementsPerLine + numLines * srcLineStride) +
       srcPlaneStride);
  size_t maxTotalPlanesOut =
      ((max_alloc_size / elementSize) + dstPlaneStride) /
  ...
  size_t inBufferSize =
      elementSize * (totalPlanes * (numLines * numElementsPerLine +
                                    numLines * srcLineStride) +
                     (totalPlanes - 1) * srcPlaneStride);
  size_t outBufferSize =
      elementSize * (totalPlanes * (numLines * numElementsPerLine +
                                    numLines * dstLineStride) +
  ...
  generate_random_data(vecType, inBufferSize / get_explicit_type_size(vecType),
                       d, inBuffer);
  generate_random_data(vecType, outBufferSize / get_explicit_type_size(vecType),
                       d, outBuffer);
  free_mtdata(d);
  d = NULL;
  ...
  for (int i = 0;
    ...
    for (int j = 0; j < (int)numLines * elementSize; j += elementSize) {
      for (int k = 0; k < (int)numElementsPerLine * elementSize;
           k += elementSize) {
        int inIdx = i * (numLines * numElementsPerLine +
                         numLines * srcLineStride + srcPlaneStride) +
                    j * (numElementsPerLine + srcLineStride) + k;
        int outIdx = i * (numLines * numElementsPerLine +
      ...
      if (j < (int)numLines * elementSize) {
        ...
        if (memcmp(((char *)outBuffer) + outIdx,
                   ...
                   dstLineStride * elementSize) != 0) {
          if (failuresPrinted == 0) {
            // Print first failure message
            log_error("ERROR: Results of copy did not validate!\n");
          }
          log_error("3D copy corrupted data in output buffer in the line "
    ...
    if (i < (int)(globalWorkgroupSize * planesCopiesPerWorkItem - 1) *
      ...
      if (memcmp(((char *)outBuffer) + outIdx, ((char *)outBufferCopy) + outIdx,
                 dstPlaneStride * elementSize) != 0) {
        if (failuresPrinted == 0) {
          // Print first failure message
          log_error("ERROR: Results of copy did not validate!\n");
        }
        log_error("3D copy corrupted data in output buffer in the plane stride "
                  "offset of plane %d\n",
                  i);
        failuresPrinted++;
      }
  ...
  for (typeIndex = 0; vecType[typeIndex] != kNumExplicitTypes; typeIndex++) {
    ...
    for (size = 0; vecSizes[size] != 0; size++) {
          ...
          2) // small type
      {
        for (srcLineStride = 0;
             srcLineStride <
             sizeof(smallTypesStrideSizes) / sizeof(smallTypesStrideSizes[0]);
             srcLineStride++) {
          for (dstLineStride = 0;
               dstLineStride <
               sizeof(smallTypesStrideSizes) / sizeof(smallTypesStrideSizes[0]);
            ...
            for (srcPlaneStride = 0;
              ...
              for (dstPlaneStride = 0;
                   dstPlaneStride < sizeof(smallTypesStrideSizes) /
                                        sizeof(smallTypesStrideSizes[0]);
                   dstPlaneStride++) {
                if (test_copy3D(deviceID, context, queue, kernelCode,
