...
int test_copy_fence(cl_device_id deviceID, cl_context context,
  ...
  if (!is_extension_available(deviceID, "cl_khr_async_work_group_copy_fence")) {
    log_info("Device does not support extended async copies fence. Skipping "
             "test.\n");
    return 0;
  }
  ...
  sprintf(
      programSource, kernelCode,
      vecType == kDouble ? "#pragma OPENCL EXTENSION cl_khr_fp64 : enable" : "",
      vecNameString, vecNameString, vecNameString, vecNameString, vecNameString,
      vecNameString, vecNameString, get_explicit_type_name(vecType),
      vecNameString, get_explicit_type_name(vecType), vecNameString,
  ...
  size_t transaction1LocalBufferSize =
      localWorkgroupSize * elementSize * transaction1NumberOfCopiesPerWorkitem;
  size_t transaction2LocalBufferSize =
      localWorkgroupSize * elementSize *
      transaction2NumberOfCopiesPerWorkitem; // irrelevant if aliased_local_mem
  ...
  log_info("Global: %d, local %d. 1st Transaction: local buffer %db, global "
  ...
  if (!transaction1DstIsTransaction2Src) {
    generate_random_data(
        vecType, transaction2GlobalBufferSize / get_explicit_type_size(vecType),
  ...
  ++argIndex;
  error =
      clSetKernelArg(kernel, argIndex, sizeof(transaction1CopiesPerWorkitemInt),
  ...
  ++argIndex;
  error =
      clSetKernelArg(kernel, argIndex, sizeof(transaction2CopiesPerWorkitemInt),
  ...
  if (transaction1DstIsTransaction2Src) {
    for (size_t idx = 0; idx < numberOfLocalWorkgroups; idx++) {
      memcpy((void *)((unsigned char *)transaction2InBuffer +
  ...
  if (transaction1SrcIsTransaction2Dst) {
    ...
    test_error(error, "Unable to read results");
    for (size_t idx = 0; idx < numberOfLocalWorkgroups; idx++) {
      memcpy((void *)((unsigned char *)transaction2OutBuffer +
                      idx * transaction2CopiesPerWorkgroup * elementSize),
  ...
  if (memcmp(transaction1InBuffer, transaction1OutBuffer,
             transaction1GlobalBufferSize) != 0) {
  ...
  if (memcmp(transaction2InBuffer, transaction2OutBuffer,
             transaction2GlobalBufferSize) != 0) {
    size_t typeSize = get_explicit_type_size(vecType) * vecSize;
