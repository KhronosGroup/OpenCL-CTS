                              ...
                              size_t size, char *type, int loops)
{
    ...
    for (i = 0; i < loops; i++)
    {
        for (int src_flag_id = 0; src_flag_id < NUM_IMMUTABLE_FLAGS;
             src_flag_id++)
        {
            clMemWrapper buffer;
            outptr[i] = align_malloc(ptrSizes[i] * num_elements, min_alignment);
            ...
            buffer =
                clCreateBuffer(context, immutable_flag_set[src_flag_id],
                               ptrSizes[i] * num_elements, outptr[i], &err);
            ...
            {
                log_error(" %s%d test failed. cl_mem_flags src: %s\n", type,
                          1 << i, immutable_flag_set_names[src_flag_id]);
                total_errors++;
            }
            else
            {
            ...
            if ((immutable_flag_set[src_flag_id] & CL_MEM_USE_HOST_PTR))
            {
                err = clFinish(queue);
                test_error(err, "clFinish failed");
            }
