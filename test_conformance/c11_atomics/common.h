...
int CBasicTest<HostAtomicType, HostDataType>::ExecuteSingleTest(cl_device_id deviceID, cl_context context, cl_command_queue queue)
{
  ...
  // This loop iteratively reduces the workgroup size by 2 and then re-generates the kernel with the reduced
  // workgroup size until we find a size which is admissible for the kernel being run or reduce the wg size
  // to the trivial case of 1 (which was separately verified to be accurate for the kernel being run)
