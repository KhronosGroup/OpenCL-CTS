...
const char *binary_fn_code_pattern = "%s\n" /* optional pragma */
                                     "__kernel void test_fn(__global %s%s *x, "
                                     "__global %s%s *y, __global %s%s *dst)\n"
                                     "{\n"
...
int test_binary_fn(cl_device_id device, cl_context context,
                   cl_command_queue queue, int n_elems, const char *fnName,
                   bool vectorSecondParam, binary_verify_float_fn floatVerifyFn,
                   binary_verify_double_fn doubleVerifyFn)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (test_double)
            input_ptr_double[i] =
                (cl_double *)malloc(sizeof(cl_double) * num_elements);
    }
    ...
    for (j = 0; j < num_elements; j++)
    {
        ...
        if (test_double)
        {
            input_ptr_double[0][j] =
                get_random_double(-0x20000000, 0x20000000, d);
            input_ptr_double[1][j] =
                get_random_double(-0x20000000, 0x20000000, d);
        }
    }
    free_mtdata(d);
    ...
    for (i = 0; i < 2; i++)
    {
        err = clEnqueueWriteBuffer(queue, streams[i], CL_TRUE, 0,
                                   ...
                                   input_ptr[i], 0, NULL, NULL);
        test_error(err, "Unable to write input buffer");

        if (test_double)
        {
            err = clEnqueueWriteBuffer(queue, streams[3 + i], CL_TRUE, 0,
                                       sizeof(cl_double) * num_elements,
                                       input_ptr_double[i], 0, NULL, NULL);
            test_error(err, "Unable to write input buffer");
        }
    }
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        ...
        if (i >= kVectorSizeCount)
        {
            ...
            {
                sprintf(programSrc, binary_fn_code_pattern_v3, "", "float",
                        "float", "float", fnName);
            }
            else
            {
                sprintf(programSrc, binary_fn_code_pattern_v3_scalar, "",
                        "float", "float", "float", fnName);
            }
        }
        else
        {
            // do regular
            sprintf(programSrc, binary_fn_code_pattern, "", "float",
                    vecSizeNames[i], "float",
                    vectorSecondParam ? vecSizeNames[i] : "", "float",
                    vecSizeNames[i], fnName);
        }
        ...
        if (test_double)
        {
            if (i >= kVectorSizeCount)
            {
                if (vectorSecondParam)
                {
                    sprintf(programSrc, binary_fn_code_pattern_v3,
            ...
            {
                sprintf(programSrc, binary_fn_code_pattern,
                        "#pragma OPENCL EXTENSION cl_khr_fp64 : enable",
                        "double", vecSizeNames[i], "double",
                        vectorSecondParam ? vecSizeNames[i] : "", "double",
                        vecSizeNames[i], fnName);
            }
            ptr = programSrc;
            err = create_single_kernel_helper(
                context, &program[kTotalVecCount + i],
                &kernel[kTotalVecCount + i], 1, &ptr, "test_fn");
            test_error(err, "Unable to create kernel");
        }
    }
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        ...
        if (floatVerifyFn(input_ptr[0], input_ptr[1], output_ptr, n_elems,
                          ((g_arrVecSizes[i]))))
        {
            log_error(" float%d%s test failed\n", ((g_arrVecSizes[i])),
                      vectorSecondParam ? "" : ", float");
            err = -1;
        }
        else
        {
    ...
    if (test_double)
    {
        for (i = 0; i < kTotalVecCount; i++)
        {
            for (j = 0; j < 3; j++)
            {
                err = clSetKernelArg(kernel[kTotalVecCount + i], j,
                                     sizeof(streams[3 + j]), &streams[3 + j]);
                test_error(err, "Unable to set kernel argument");
            }

            threads[0] = (size_t)n_elems;

            err = clEnqueueNDRangeKernel(queue, kernel[kTotalVecCount + i], 1,
                                         NULL, threads, NULL, 0, NULL, NULL);
            test_error(err, "Unable to execute kernel");

            err = clEnqueueReadBuffer(queue, streams[5], CL_TRUE, 0,
                                      sizeof(cl_double) * num_elements,
                                      (void *)output_ptr_double, 0, NULL, NULL);
            test_error(err, "Unable to read results");

            if (doubleVerifyFn(input_ptr_double[0], input_ptr_double[1],
                               output_ptr_double, n_elems,
                               ((g_arrVecSizes[i]))))
            {
                log_error(" double%d%s test failed\n", ((g_arrVecSizes[i])),
                          vectorSecondParam ? "" : ", double");
                err = -1;
            }
            else
            {
