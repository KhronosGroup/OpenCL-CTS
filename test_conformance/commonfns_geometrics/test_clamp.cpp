...
static int verify_clamp(float *x, float *minval, float *maxval, float *outptr,
                        int n)
{
    float t;
    int i;
...
static int verify_clamp_double(double *x, double *minval, double *maxval,
                               double *outptr, int n)
{
    double t;
    int i;
               ...
               int n_elems)
{
    ...
    for (j = 0; j < num_elements; j++)
    {
        ...
        if (test_double)
        {
            ...
            input_ptr_double[1][j] =
                get_random_double(-0x20000000, 0x20000000, d);
            input_ptr_double[2][j] =
                get_random_double(input_ptr_double[1][j], 0x20000000, d);
        }
    }
    free_mtdata(d);
    d = NULL;
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (test_double)
        {
            err = clEnqueueWriteBuffer(queue, streams[4 + i], CL_TRUE, 0,
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        err = create_single_kernel_helper(context, &program[i], &kernel[i], 1,
                                          &clamp_float_codes[i], "test_clamp");
        ...
        if (test_double)
        {
            err = create_single_kernel_helper(
                ...
                &kernel[kTotalVecCount + i], 1, &clamp_double_codes[i],
                "test_clamp");
            log_info(
                "Just made a program for double, i=%d, size=%d, in slot %d\n",
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        ...
        if (verify_clamp(input_ptr[0], input_ptr[1], input_ptr[2], output_ptr,
    ...
    if (test_double)
    {
        for (; i < 2 * kTotalVecCount; i++)
        {
            ...
            for (j = 0; j < 4; j++)
            {
                err = clSetKernelArg(kernel[i], j, sizeof(streams[j + 4]),
                                     &streams[j + 4]);
                test_error(err, "Unable to set kernel argument");
            }
            ...
            err = clEnqueueNDRangeKernel(queue, kernel[i], 1, NULL, threads,
                                         NULL, 0, NULL, NULL);
            test_error(err, "Unable to execute kernel");

            err = clEnqueueReadBuffer(queue, streams[7], CL_TRUE, 0,
                                      sizeof(cl_double) * num_elements,
                                      (void *)output_ptr_double, 0, NULL, NULL);
            test_error(err, "Unable to read results");

            if (verify_clamp_double(input_ptr_double[0], input_ptr_double[1],
                                    input_ptr_double[2], output_ptr_double,
                                    n_elems
    ...
    for (i = 0; i < ((test_double) ? kTotalVecCount * 2 - 1 : kTotalVecCount);
         i++)
    {
