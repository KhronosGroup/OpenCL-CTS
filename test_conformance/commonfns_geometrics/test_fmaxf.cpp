...
static int verify_fmax(float *inptrA, float *inptrB, float *outptr, int n,
                       int veclen)
{
    float r;
    int i, j;
    ...
    for (i = 0; i < n;)
    {
        ...
        for (j = 0; j < veclen && i < n; ++j, ++i)
        {
            ...
            {
                log_info(
                    "Verify noted discrepancy at %d (of %d) (vec %d, pos %d)\n",
                    i, n, ii, j);
               ...
               int n_elems)
{
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        ...
        err = clEnqueueReadBuffer(queue, streams[2], true, 0,
                                  sizeof(cl_float) * num_elements, output_ptr,
                                  0, NULL, NULL);
        if (err != CL_SUCCESS)
        {
        ...
        if (verify_fmax(input_ptr[0], input_ptr[1], output_ptr,
