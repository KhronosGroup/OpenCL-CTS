...
const char *crossKernelSource =
    "__kernel void sample_test(__global float4 *sourceA, __global float4 "
    "*sourceB, __global float4 *destValues)\n"
    "{\n"
...
const char *crossKernelSourceV3 =
    "__kernel void sample_test(__global float *sourceA, __global float "
    "*sourceB, __global float *destValues)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "    vstore3( cross( vload3( tid, sourceA), vload3( tid,  sourceB) ), tid, "
    "destValues );\n"
    "\n"
    "}\n";

const char *twoToFloatKernelPattern =
    "__kernel void sample_test(__global float%s *sourceA, __global float%s "
    "*sourceB, __global float *destValues)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
...
const char *oneToFloatKernelPattern =
    "__kernel void sample_test(__global float%s *sourceA, __global float "
    "*destValues)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
...
const char *oneToOneKernelPatternV3 =
    ...
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "    vstore3( %s( vload3( tid, (__global float*) sourceA) ), tid, "
...
void vector2string(char *string, float *vector, size_t elements)
{
    ...
    *string++ = ' ';
    ...
    size_t i;
    for (i = 1; i < elements; i++) string += sprintf(string, ", %a", vector[i]);
    *string++ = ' ';
    *string++ = '}';
    *string = '\0';
}
...
{
    static const cl_float trickyValues[] = {
        -FLT_EPSILON,
        FLT_EPSILON,
    ...
    static const size_t trickyCount =
        sizeof(trickyValues) / sizeof(trickyValues[0]);
    static const size_t stride[4] = { 1, trickyCount, trickyCount * trickyCount,
                                      trickyCount * trickyCount * trickyCount };
    size_t i, j, k;

    for (j = 0; j < vecSize; j++)
        for (k = 0; k < vecSize; k++)
            for (i = 0; i < trickyCount; i++)
                aVectors[j + stride[j] * (i + k * trickyCount) * vecSize] =
                    trickyValues[i];
...
void cross_product(const float *vecA, const float *vecB, float *outVector,
                   float *errorTolerances, float ulpTolerance)
{
    ...
    errorTolerances[0] =
        fmaxf(fabsf(vecA[1]),
              fmaxf(fabsf(vecB[2]), fmaxf(fabsf(vecA[2]), fabsf(vecB[1]))));
    errorTolerances[1] =
        fmaxf(fabsf(vecA[2]),
              fmaxf(fabsf(vecB[0]), fmaxf(fabsf(vecA[0]), fabsf(vecB[2]))));
    errorTolerances[2] =
        fmaxf(fabsf(vecA[0]),
              fmaxf(fabsf(vecB[1]), fmaxf(fabsf(vecA[1]), fabsf(vecB[0]))));

    errorTolerances[0] = errorTolerances[0] * errorTolerances[0]
        * (ulpTolerance
           * FLT_EPSILON); // This gives us max squared times ulp tolerance,
                           // i.e. the worst-case expected variance we could
                           // expect from this result
    errorTolerances[1] =
        errorTolerances[1] * errorTolerances[1] * (ulpTolerance * FLT_EPSILON);
    errorTolerances[2] =
        errorTolerances[2] * errorTolerances[2] * (ulpTolerance * FLT_EPSILON);
}


int test_geom_cross(cl_device_id deviceID, cl_context context,
                    cl_command_queue queue, int num_elements)
{
    ...
    cl_device_fp_config defaultRoundingMode =
        get_default_rounding_mode(deviceID);
    if (0 == defaultRoundingMode) return -1;
    ...
    for (vecsize = 3; vecsize <= 4; ++vecsize)
    {
        ...
        BufferOwningPtr<cl_float> A(
            malloc(sizeof(cl_float) * TEST_SIZE * vecsize));
        BufferOwningPtr<cl_float> B(
            malloc(sizeof(cl_float) * TEST_SIZE * vecsize));
        BufferOwningPtr<cl_float> C(
            malloc(sizeof(cl_float) * TEST_SIZE * vecsize));
        ...
        error = clEnqueueReadBuffer(queue, streams[2], true, 0,
                                    sizeof(cl_float) * TEST_SIZE * vecsize,
                                    outData, 0, NULL, NULL);
        ...
        for (i = 0; i < TEST_SIZE; i++)
        {
            ...
            cross_product(inDataA + i * vecsize, inDataB + i * vecsize,
                          testVector, errorTolerances, 3.f);
            ...
            {
                log_error("ERROR: Data sample %d does not validate! Expected "
                          "(%a,%a,%a,%a), got (%a,%a,%a,%a)\n",
                ...
                log_error("    Input: (%a %a %a) and (%a %a %a)\n",
                          inDataA[i * vecsize + 0], inDataA[i * vecsize + 1],
                          inDataA[i * vecsize + 2], inDataB[i * vecsize + 0],
                          inDataB[i * vecsize + 1], inDataB[i * vecsize + 2]);
                log_error("    Errors: (%a out of %a), (%a out of %a), (%a out "
                          "of %a)\n",
                          errs[0], errorTolerances[0], errs[1],
                          errorTolerances[1], errs[2], errorTolerances[2]);
                log_error("     ulp %f\n",
    ...
    else
    {
        ...
        return test_geom_cross_double(deviceID, context, queue, num_elements,
                                      seed);
    }
}
...
int test_twoToFloat_kernel(cl_command_queue queue, cl_context context,
                           const char *fnName, size_t vecSize,
                           twoToFloatVerifyFn verifyFn, float ulpLimit,
                           MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    ...
    error = clGetDeviceInfo(device, CL_DEVICE_PROFILE, sizeof(kernelSource),
                            kernelSource, NULL);
    ...
    if (0 == strcmp(kernelSource, "EMBEDDED_PROFILE"))
    {
        ...
        if (CL_FP_ROUND_TO_ZERO
            == (config & (CL_FP_ROUND_TO_NEAREST | CL_FP_ROUND_TO_ZERO)))
            ulpLimit *= 2.0f; // rtz operations average twice the accrued error
                              // of rte operations

        if (0 == (config & CL_FP_INF_NAN)) hasInfNan = 0;
    }
    ...
    sprintf(kernelSource,
            vecSize == 3 ? twoToFloatKernelPatternV3 : twoToFloatKernelPattern,
            sizeNames[vecSize - 1], sizeNames[vecSize - 1], fnName);
    ...
    if (verifyFn == verifyFastDistance)
    {
        for (i = 0; i < TEST_SIZE * vecSize; i++)
        {
            if (fabsf(inDataA[i]) > MAKE_HEX_FLOAT(0x1.0p62f, 0x1L, 62)
                || fabsf(inDataA[i]) < MAKE_HEX_FLOAT(0x1.0p-62f, 0x1L, -62))
                inDataA[i] = get_random_float(-512.f, 512.f, d);
            if (fabsf(inDataB[i]) > MAKE_HEX_FLOAT(0x1.0p62f, 0x1L, 62)
                || fabsf(inDataB[i]) < MAKE_HEX_FLOAT(0x1.0p-62f, 0x1L, -62))
                inDataB[i] = get_random_float(-512.f, 512.f, d);
        }
    }
    ...
    /* Now get the results */
    error = clEnqueueReadBuffer(queue, streams[2], true, 0,
                                sizeof(cl_float) * TEST_SIZE, outData, 0, NULL,
                                NULL);
    test_error(error, "Unable to read output array!");
    ...
    for (i = 0; i < TEST_SIZE; i++)
    {
        ...
        {
            if (isnan(expected) && isnan(outData[i])) continue;
            ...
            if (ulpLimit < 0)
            {
                ...
                float maxValue = getMaxValue(inDataA + i * vecSize,
                                             inDataB + i * vecSize, vecSize);
                // In this case (dot is the only one that gets here), the ulp is
                // 2*vecSize - 1 (n + n-1 max # of errors)
                float errorTolerance = maxValue * maxValue
                ...
                if (error > errorTolerance)
                {
                    ...
                    log_error("ERROR: Data sample %d at size %d does not "
                              "validate! Expected (%a), got (%a), sources (%a "
                              "and %a) error of %g against tolerance %g\n",
                              (int)i, (int)vecSize, expected, outData[i],
                              inDataA[i * vecSize], inDataB[i * vecSize],
                              (float)error, (float)errorTolerance);
...
int test_geom_dot(cl_device_id deviceID, cl_context context,
                  cl_command_queue queue, int num_elements)
{
    ...
    {
        if (test_twoToFloat_kernel(queue, context, "dot", sizes[size],
                                   verifyDot, -1.0f /*magic value*/, seed)
            != 0)
        {
...
int test_geom_fast_distance(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        float maxUlps = 8192.0f + // error in sqrt
            (1.5f * (float)sizes[size]
             + // cumulative error for multiplications  (a-b+0.5ulp)**2 =
               // (a-b)**2 + a*0.5ulp + b*0.5 ulp + 0.5 ulp for multiplication
             0.5f * (float)(sizes[size] - 1)); // cumulative error for additions

        if (test_twoToFloat_kernel(queue, context, "fast_distance", sizes[size],
                                   verifyFastDistance, maxUlps, seed)
            != 0)
        {
...
int test_geom_distance(cl_device_id deviceID, cl_context context,
                       cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        if (test_twoToFloat_kernel(queue, context, "distance", sizes[size],
                                   verifyDistance, maxUlps, seed)
            != 0)
        {
    ...
    {
        log_info("Testing doubles...\n");
        return test_geom_distance_double(deviceID, context, queue, num_elements,
                                         seed);
    }
}
...
int test_oneToFloat_kernel(cl_command_queue queue, cl_context context,
                           const char *fnName, size_t vecSize,
                           oneToFloatVerifyFn verifyFn, float ulpLimit,
                           MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    ...
    /* Create the source */
    sprintf(kernelSource,
            vecSize == 3 ? oneToFloatKernelPatternV3 : oneToFloatKernelPattern,
            sizeNames[vecSize - 1], fnName);
    ...
    for (i = 0; i < TEST_SIZE; i++)
    {
        ...
        if ((float)expected != outData[i])
        {
            ...
            if (fabsf(ulps) <= ulpLimit) continue;

            // We have to special case NAN
            if (isnan(outData[i]) && isnan(expected)) continue;

            if (!(fabsf(ulps) < ulpLimit))
            {
                log_error("ERROR: Data sample %d at size %d does not validate! "
                          "Expected (%a), got (%a), source (%a), ulp %f\n",
                          (int)i, (int)vecSize, expected, outData[i],
                          inDataA[i * vecSize], ulps);
                char vecA[1000];
                vector2string(vecA, inDataA + i * vecSize, vecSize);
...
int test_geom_length(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        float maxUlps = 3.0f + // error in sqrt
            0.5f * // effect on e of taking sqrt( x + e )
                (0.5f * (float)sizes[size]
                 + // cumulative error for multiplications
                 0.5f
                     * (float)(sizes[size]
                               - 1)); // cumulative error for additions

        if (test_oneToFloat_kernel(queue, context, "length", sizes[size],
                                   verifyLength, maxUlps, seed)
            != 0)
        {
    ...
    {
        log_info("Testing doubles...\n");
        return test_geom_length_double(deviceID, context, queue, num_elements,
                                       seed);
    }
}
...
int test_geom_fast_length(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        if (test_oneToFloat_kernel(queue, context, "fast_length", sizes[size],
                                   verifyFastLength, maxUlps, seed)
            != 0)
        {
...
int test_oneToOne_kernel(cl_command_queue queue, cl_context context,
                         const char *fnName, size_t vecSize,
                         oneToOneVerifyFn verifyFn, float ulpLimit,
                         int softball, MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    clMemWrapper streams[2];
    ...
    /* Create the source */
    sprintf(kernelSource,
            vecSize == 3 ? oneToOneKernelPatternV3 : oneToOneKernelPattern,
            sizeNames[vecSize - 1], sizeNames[vecSize - 1], fnName);
    ...
    { // keep problematic cases out of the fast function
        ...
        {
            cl_float z = get_random_float(-MAKE_HEX_FLOAT(0x1.0p60f, 1, 60),
    ...
    /* Now get the results */
    error = clEnqueueReadBuffer(queue, streams[1], true, 0,
                                sizeof(cl_float) * TEST_SIZE * vecSize, outData,
                                0, NULL, NULL);
    test_error(error, "Unable to read output array!");
    ...
    for (i = 0; i < TEST_SIZE; i++)
    {
        ...
        {
            // We have to special case NAN
            if (isnan(outData[i * vecSize + j]) && isnan(expected[j])) continue;
        ...
        if (fail && gFlushDenormsToZero)
        {
            ...
            for (j = 0; j < vecSize; j++)
            {
                ...
                {
                    ulp_error =
                        Ulp_Error(outData[i * vecSize + j], expected[j]);
                    ...
                    if (fabsf(ulp_error) > ulpLimit)
                    {
                        if (IsFloatSubnormal(expected2[j]))
                        {
                            ...
                            if (expected2[j] != outData[i * vecSize + j])
                            {
                                ulp_error = Ulp_Error(outData[i * vecSize + j],
                                                      expected[j]);
                                if (fabsf(ulp_error) > ulpLimit)
                                {
                                    fail = 1;
        ...
        if (fail)
        {
            log_error(
                ...
                (int)i, (int)j, (int)vecSize, expected[j], expected[j],
                outData[i * vecSize + j], outData[i * vecSize + j], ulp_error);
            log_error("       Source: ");
            ...
            log_error("\n");
            log_error("       Expected: ");
            for (size_t q = 0; q < vecSize; q++) log_error("%g ", expected[q]);
            log_error("\n             : ");
            for (size_t q = 0; q < vecSize; q++) log_error("%a ", expected[q]);
            log_error("\n");
            return -1;
        }
    }
...
void verifyNormalize(float *srcA, float *dst, size_t vecSize)
{
    ...
    for (i = 0; i < vecSize; i++) total += (double)srcA[i] * (double)srcA[i];

    if (total == 0.f)
    {
        // Special edge case: copy vector over without change
        for (i = 0; i < vecSize; i++) dst[i] = srcA[i];
        return;
    }
    ...
    value = sqrt(total);
    for (i = 0; i < vecSize; i++) dst[i] = (float)((double)srcA[i] / value);
}

int test_geom_normalize(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        float maxUlps = 2.5f + // error in rsqrt + error in multiply
            (0.5f * (float)sizes[size] + // cumulative error for multiplications
             0.5f * (float)(sizes[size] - 1)); // cumulative error for additions
        if (test_oneToOne_kernel(queue, context, "normalize", sizes[size],
                                 verifyNormalize, maxUlps, 0, seed)
            != 0)
        {
    ...
    {
        log_info("Testing doubles...\n");
        return test_geom_normalize_double(deviceID, context, queue,
                                          num_elements, seed);
    }
}


int test_geom_fast_normalize(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        if (test_oneToOne_kernel(queue, context, "fast_normalize", sizes[size],
                                 verifyNormalize, maxUlps, 1, seed)
            != 0)
        {
