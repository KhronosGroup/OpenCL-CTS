...
const char *crossKernelSource_double =
    ...
    "__kernel void sample_test(__global double4 *sourceA, __global double4 "
    "*sourceB, __global double4 *destValues)\n"
    "{\n"
...
const char *crossKernelSource_doubleV3 =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void sample_test(__global double *sourceA, __global double "
    "*sourceB, __global double *destValues)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
...
const char *twoToFloatKernelPattern_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void sample_test(__global double%s *sourceA, __global double%s "
    "*sourceB, __global double *destValues)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
...
const char *oneToFloatKernelPattern_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void sample_test(__global double%s *sourceA, __global double "
    "*destValues)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
...
const char *oneToOneKernelPattern_doubleV3 =
    ...
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "    vstore3( %s( vload3( tid, (__global double*) sourceA) ), tid, "
...
void fillWithTrickyNumbers_double(double *aVectors, double *bVectors,
                                  size_t vecSize)
{
    static const cl_double trickyValues[] = {
        -FLT_EPSILON,
        FLT_EPSILON,
...
void cross_product_double(const double *vecA, const double *vecB,
                          double *outVector, double *errorTolerances,
                          double ulpTolerance)
{
    ...
    errorTolerances[0] = fmax(
        fabs(vecA[1]), fmax(fabs(vecB[2]), fmax(fabs(vecA[2]), fabs(vecB[1]))));
    errorTolerances[1] = fmax(
        fabs(vecA[2]), fmax(fabs(vecB[0]), fmax(fabs(vecA[0]), fabs(vecB[2]))));
    errorTolerances[2] = fmax(
        fabs(vecA[0]), fmax(fabs(vecB[1]), fmax(fabs(vecA[1]), fabs(vecB[0]))));
...
int test_geom_cross_double(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements, MTdata d)
{
    cl_int error;
    ...
    adjustment =
        32 * 1024 * 1024; /* Try to allocate a bit less than the limits */
    for (vecsize = 3; vecsize <= 4; ++vecsize)
    {
        ...
        while ((bufSize > (maxAllocSize - adjustment))
               || (3 * bufSize > (maxGlobalMemSize - adjustment)))
        {
            size /= 2;
        ...
        if (create_single_kernel_helper(context, &program, &kernel, 1,
                                        vecsize == 3
                                            ? &crossKernelSource_doubleV3
                                            : &crossKernelSource_double,
                                        "sample_test"))
            return -1;
        ...
        /* Now get the results */
        error = clEnqueueReadBuffer(queue, streams[2], true, 0, bufSize,
                                    outData, 0, NULL, NULL);
        ...
        for (i = 0; i < size; i++)
        {
            ...
            cross_product_double(inDataA + i * vecsize, inDataB + i * vecsize,
...
typedef double (*twoToFloatVerifyFn_double)(double *srcA, double *srcB,
                                            size_t vecSize);

int test_twoToFloat_kernel_double(cl_command_queue queue, cl_context context,
                                  const char *fnName, size_t vecSize,
                                  twoToFloatVerifyFn_double verifyFn,
                                  double ulpLimit, MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    clMemWrapper streams[3];
    ...
    char sizeNames[][4] = { "", "2", "3", "4", "", "", "", "8",
                            "", "",  "",  "",  "", "", "", "16" };
    BufferOwningPtr<cl_double> A(
        malloc(sizeof(cl_double) * TEST_SIZE * vecSize));
    BufferOwningPtr<cl_double> B(
        malloc(sizeof(cl_double) * TEST_SIZE * vecSize));
    ...
    sprintf(kernelSource,
            vecSize == 3 ? twoToFloatKernelPattern_doubleV3
                         : twoToFloatKernelPattern_double,
    ...
    error = clEnqueueReadBuffer(queue, streams[2], true, 0,
                                sizeof(cl_double) * TEST_SIZE, outData, 0, NULL,
                                NULL);
    test_error(error, "Unable to read output array!");
    ...
    {
        double expected =
            verifyFn(inDataA + i * vecSize, inDataB + i * vecSize, vecSize);
        if ((double)expected != outData[i])
        {
            ...
            if (ulpLimit < 0)
            {
                ...
                double maxValue = getMaxValue_double(
                    inDataA + i * vecSize, inDataB + i * vecSize, vecSize);

                // In this case (dot is the only one that gets here), the ulp is
                // 2*vecSize - 1 (n + n-1 max # of errors)
                double errorTolerance = maxValue * maxValue
                    * (2.f * (double)vecSize - 1.f) * FLT_EPSILON;
                ...
                if (error > errorTolerance)
                {
                    ...
                    log_error("ERROR: Data sample %d at size %d does not "
                              ...
                              (double)error, (double)errorTolerance);
            ...
            {
                double error = Ulp_Error_Double(outData[i], expected);
...
int test_geom_dot_double(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, int num_elements, MTdata d)
{
    ...
    {
        if (test_twoToFloat_kernel_double(queue, context, "dot", sizes[size],
                                          verifyDot_double,
                                          -1.0f /*magic value*/, d)
            != 0)
        {
...
int test_geom_fast_distance_double(cl_device_id deviceID, cl_context context,
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        double maxUlps = 8192.0f + // error in sqrt
            0.5f * // effect on e of taking sqrt( x + e )
                (1.5f * (double)sizes[size]
                 + // cumulative error for multiplications  (a-b+0.5ulp)**2 =
                   // (a-b)**2 + a*0.5ulp + b*0.5 ulp + 0.5 ulp for
                   // multiplication
                 0.5f
                     * (double)(sizes[size]
                                - 1)); // cumulative error for additions

        if (test_twoToFloat_kernel_double(queue, context, "fast_distance",
                                          sizes[size], verifyDistance_double,
                                          maxUlps, d)
            != 0)
        {
...
double verifyDistance_double(double *srcA, double *srcB, size_t vecSize)
{
    ...
    for (i = 0; i < vecSize; i++) diff[i] = srcA[i] - srcB[i];

    return verifyLength_double(diff, vecSize);
}

int test_geom_distance_double(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements,
                              MTdata d)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        if (test_twoToFloat_kernel_double(queue, context, "distance",
                                          sizes[size], verifyDistance_double,
                                          maxUlps, d)
            != 0)
        {
...
int test_oneToFloat_kernel_double(cl_command_queue queue, cl_context context,
                                  const char *fnName, size_t vecSize,
                                  oneToFloatVerifyFn_double verifyFn,
                                  double ulpLimit, MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    ...
    /* Create the source */
    sprintf(kernelSource,
            vecSize == 3 ? oneToFloatKernelPattern_doubleV3
                         : oneToFloatKernelPattern_double,
    ...
    /* Generate some streams */
    for (i = 0; i < TEST_SIZE * vecSize; i++) inDataA[i] = any_double(d);
    ...
    for (i = 0; i < TEST_SIZE; i++)
    {
        ...
        if ((double)expected != outData[i])
        {
            ...
            if (fabs(ulps) <= ulpLimit) continue;
...
double verifyLength_double(double *srcA, size_t vecSize)
{
    ...
    for (i = 0; i < vecSize; i++) total += srcA[i] * srcA[i];
...
int test_geom_length_double(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements, MTdata d)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    unsigned int size;
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        maxUlps *= 2.0; // our reference code may be in error too
        if (test_oneToFloat_kernel_double(queue, context, "length", sizes[size],
                                          verifyLength_double, maxUlps, d)
            != 0)
        {
...
int test_geom_fast_length_double(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements,
                                 MTdata d)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        if (test_oneToFloat_kernel_double(queue, context, "fast_length",
                                          sizes[size], verifyFastLength_double,
                                          maxUlps, d)
            != 0)
        {
...
typedef void (*oneToOneVerifyFn_double)(double *srcA, double *dstA,
                                        size_t vecSize);

int test_oneToOne_kernel_double(cl_command_queue queue, cl_context context,
                                const char *fnName, size_t vecSize,
                                oneToOneVerifyFn_double verifyFn,
                                double ulpLimit, MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    ...
    /* Create the source */
    sprintf(kernelSource,
            vecSize == 3 ? oneToOneKernelPattern_doubleV3
                         : oneToOneKernelPattern_double,
            sizeNames[vecSize - 1], sizeNames[vecSize - 1], fnName);
    ...
    for (i = vecSize; i < TEST_SIZE * vecSize; i++) inDataA[i] = any_double(d);
    ...
    /* Now get the results */
    error = clEnqueueReadBuffer(queue, streams[1], true, 0,
                                sizeof(cl_double) * TEST_SIZE * vecSize,
                                outData, 0, NULL, NULL);
    test_error(error, "Unable to read output array!");
    ...
    for (i = 0; i < TEST_SIZE; i++)
    {
        ...
        for (j = 0; j < vecSize; j++)
        {
            ...
            if (expected[j] != outData[i * vecSize + j])
            {
                ...
                if (fabs(error) > ulpLimit)
                {
                    log_error("ERROR: Data sample {%d,%d} at size %d does not "
                              "validate! Expected %12.24f (%a), got %12.24f "
                              "(%a), ulp %f\n",
                              (int)i, (int)j, (int)vecSize, expected[j],
                              expected[j], outData[i * vecSize + j],
...
void verifyNormalize_double(double *srcA, double *dst, size_t vecSize)
{
    ...
    for (i = 0; i < vecSize; i++) dst[i] = srcA[i] / value;
}

int test_geom_normalize_double(cl_device_id deviceID, cl_context context,
                               cl_command_queue queue, int num_elements,
                               MTdata d)
{
    size_t sizes[] = { 1, 2, 3, 4, 0 };
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        maxUlps *= 2.0; // our reference code is not infinitely precise and may
                        // have error of its own
        if (test_oneToOne_kernel_double(queue, context, "normalize",
                                        sizes[size], verifyNormalize_double,
                                        maxUlps, d)
            != 0)
        {
