...
static int test_radians_double(cl_device_id device, cl_context context,
...
const char *radians16_kernel_code = "__kernel void test_radians16(__global "
                                    "float16 *src, __global float16 *dst)\n"
                                    "{\n"
                                    "    int  tid = get_global_id(0);\n"
                                    "\n"
                                    "    dst[tid] = radians(src[tid]);\n"
...
static float verify_radians(float *inptr, float *outptr, int n)
{
                 ...
                 cl_command_queue queue, int n_elems)
{
    ...
    {
        p[i] = get_random_float((float)(-100000.f * M_PI),
    ...
    {
        log_info("Skipping double -- cl_khr_fp64 is not supported by this "
                 "device.\n");
        return 0;
    }
...
const char *radians_kernel_code_double =
    ...
    "__kernel void test_radians_double(__global double *src, __global double "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *radians2_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_radians2_double(__global double2 *src, __global "
    "double2 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *radians4_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_radians4_double(__global double4 *src, __global "
    "double4 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *radians8_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_radians8_double(__global double8 *src, __global "
    "double8 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *radians16_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_radians16_double(__global double16 *src, __global "
    "double16 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *radians3_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_radians3_double(__global double *src, __global double "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
static double verify_radians_double(double *inptr, double *outptr, int n)
{
    float error, max_error = 0.0f;
    double r, max_val = NAN;
    int i, j, max_index = 0;
...
int test_radians_double(cl_device_id device, cl_context context,
                        cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    d = init_genrand(gRandomSeed);
    for (i = 0; i < num_elements; i++)
        p[i] = get_random_double((float)(-100000.0 * M_PI),
                                 (float)(100000.0 * M_PI), d);
    ...
    err = create_single_kernel_helper(context, &program[0], &kernel[0], 1,
                                      &radians_kernel_code_double,
                                      "test_radians_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                      &radians2_kernel_code_double,
                                      "test_radians2_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                      &radians4_kernel_code_double,
                                      "test_radians4_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[3], &kernel[3], 1,
                                      &radians8_kernel_code_double,
                                      "test_radians8_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[4], &kernel[4], 1,
                                      &radians16_kernel_code_double,
                                      "test_radians16_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[5], &kernel[5], 1,
                                      &radians3_kernel_code_double,
