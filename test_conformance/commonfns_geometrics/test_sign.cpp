...
static int test_sign_double(cl_device_id device, cl_context context,
...
static int verify_sign(float *inptr, float *outptr, int n)
{
    float r;
    int i;
...
static const char *fn_names[] = {
    "SIGN float",  "SIGN float2",  "SIGN float4",
    "SIGN float8", "SIGN float16", "SIGN float3"
};
...
const char *sign_kernel_code_double =
    ...
    "__kernel void test_sign_double(__global double *src, __global double "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *sign2_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_sign2_double(__global double2 *src, __global double2 "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *sign4_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_sign4_double(__global double4 *src, __global double4 "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *sign8_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_sign8_double(__global double8 *src, __global double8 "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *sign16_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_sign16_double(__global double16 *src, __global "
    "double16 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
const char *sign3_kernel_code_double =
    "#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
    "__kernel void test_sign3_double(__global double *src, __global double "
    "*dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
static int verify_sign_double(double *inptr, double *outptr, int n)
{
    double r;
    int i;
...
static const char *fn_names_double[] = { "SIGN double",   "SIGN double2",
                                         "SIGN double4",  "SIGN double8",
                                         "SIGN double16", "SIGN double3" };

int test_sign_double(cl_device_id device, cl_context context,
                     cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    err = create_single_kernel_helper(context, &program[0], &kernel[0], 1,
                                      &sign_kernel_code_double,
                                      "test_sign_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                      &sign2_kernel_code_double,
                                      "test_sign2_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                      &sign4_kernel_code_double,
                                      "test_sign4_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[3], &kernel[3], 1,
                                      &sign8_kernel_code_double,
                                      "test_sign8_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[4], &kernel[4], 1,
                                      &sign16_kernel_code_double,
                                      "test_sign16_double");
    if (err) return -1;
    err = create_single_kernel_helper(context, &program[5], &kernel[5], 1,
                                      &sign3_kernel_code_double,
                                      "test_sign3_double");
    if (err) return -1;
