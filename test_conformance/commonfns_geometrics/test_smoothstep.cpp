...
static const char *smoothstep_kernel_code =
    "__kernel void test_smoothstep(__global float *edge0, __global float "
    "*edge1, __global float *x, __global float *dst)\n"
    "{\n"
...
static const char *smoothstep2_kernel_code =
    "__kernel void test_smoothstep2(__global float2 *edge0, __global float2 "
    "*edge1, __global float2 *x, __global float2 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
    "    dst[tid] = smoothstep(edge0[tid], edge1[tid], x[tid]);\n"
...
static const char *smoothstep4_kernel_code =
    "__kernel void test_smoothstep4(__global float4 *edge0, __global float4 "
    "*edge1, __global float4 *x, __global float4 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
    "    dst[tid] = smoothstep(edge0[tid], edge1[tid], x[tid]);\n"
...
static const char *smoothstep8_kernel_code =
    "__kernel void test_smoothstep8(__global float8 *edge0, __global float8 "
    "*edge1, __global float8 *x, __global float8 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
    "    dst[tid] = smoothstep(edge0[tid], edge1[tid], x[tid]);\n"
...
static const char *smoothstep16_kernel_code =
    "__kernel void test_smoothstep16(__global float16 *edge0, __global float16 "
    "*edge1, __global float16 *x, __global float16 *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
    "    dst[tid] = smoothstep(edge0[tid], edge1[tid], x[tid]);\n"
...
static const char *smoothstep3_kernel_code =
    "__kernel void test_smoothstep3(__global float *edge0, __global float "
    "*edge1, __global float *x, __global float *dst)\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "\n"
...
static float verify_smoothstep(float *edge0, float *edge1, float *x,
                               float *outptr, int n)
{
...
const static char *fn_names[] = { "SMOOTHSTEP float",   "SMOOTHSTEP float2",
                                  "SMOOTHSTEP float4",  "SMOOTHSTEP float8",
                                  "SMOOTHSTEP float16", "SMOOTHSTEP float3" };
                    ...
                    cl_command_queue queue, int n_elems)
{
    ...
    err = clEnqueueWriteBuffer(queue, streams[2], true, 0,
                               sizeof(cl_float) * num_elements,
                               (void *)input_ptr[2], 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        log_error("clWriteArray failed\n");
        return -1;
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        ...
        max_err = verify_smoothstep(input_ptr[0], input_ptr[1], input_ptr[2],
                                    output_ptr, n_elems * g_arrVecSizes[i]);
