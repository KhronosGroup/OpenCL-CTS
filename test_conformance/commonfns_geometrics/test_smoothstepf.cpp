...
static const char *smoothstep2_kernel_code =
    "__kernel void test_smoothstep2f(__global float *edge0, __global float "
...
static const char *smoothstep4_kernel_code =
    "__kernel void test_smoothstep4f(__global float *edge0, __global float "
                     ...
                     cl_command_queue queue, int n_elems)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        switch (i)
        {
            case 0:
                max_err =
                    verify_smoothstep(input_ptr[0], input_ptr[1], input_ptr[2],
                                      output_ptr, n_elems, 1);
                break;
            case 1:
                max_err =
                    verify_smoothstep(input_ptr[0], input_ptr[1], input_ptr[2],
                                      output_ptr, n_elems, 2);
                break;
            case 2:
                max_err =
                    verify_smoothstep(input_ptr[0], input_ptr[1], input_ptr[2],
                                      output_ptr, n_elems, 4);
                break;
        }
