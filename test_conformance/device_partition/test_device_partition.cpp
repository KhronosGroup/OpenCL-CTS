...
{
    cl_device_partition_property supportedProps[3]; // only 3 types defined in the spec
    cl_uint i;
    int err = clGetDeviceInfo(parentDevice, CL_DEVICE_PARTITION_PROPERTIES, sizeof(supportedProps), &supportedProps, NULL);
    test_error( err, "Unable to get supported partition properties" );
    for (i = 0; i < 3; i++)
    {
