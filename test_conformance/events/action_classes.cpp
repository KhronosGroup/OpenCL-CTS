...
cl_int    Action::IGetPreferredImageSize2D( cl_device_id device, size_t &outWidth, size_t &outHeight )
{
    ...
    if( maxWidth * maxHeight * 4 > maxAllocSize / 5 )
    {
        float rootSize = sqrtf( (float)( maxAllocSize / ( 5 * 4 ) ) );

        if( (size_t)rootSize > maxWidth )
            outWidth = maxWidth;
        else
            outWidth = (size_t)rootSize;
        outHeight = (size_t)( ( maxAllocSize / ( 5 * 4 ) ) / outWidth );
...
cl_int    Action::IGetPreferredImageSize3D( cl_device_id device, size_t &outWidth, size_t &outHeight, size_t &outDepth )
{
    ...
    // Create something of a decent size
    if( (cl_ulong)maxWidth * maxHeight * maxDepth > maxAllocSize / ( 5 * 4 ) )
    {
        float rootSize = cbrtf( (float)( maxAllocSize / ( 5 * 4 ) ) );
        ...
        outDepth = (size_t)( ( maxAllocSize / ( 5 * 4 ) ) / ( outWidth * outHeight ) );
