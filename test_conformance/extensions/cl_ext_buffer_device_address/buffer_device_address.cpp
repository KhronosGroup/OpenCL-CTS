...
REGISTER_TEST(private_address_multi_device)
{
    ...
    for (cl_mem_device_address_ext a : addresses)
    {
        if (a == 0) test_fail("Unexpected null cl_mem_device_address_ext\n");
    }
    ...
    for (cl_uint i = 0; i < numDevices; ++i)
    {
        ...
        error = clSetKernelArgDevicePointerEXT(kernel, 1,
                                               addresses[(i + 1) % numDevices]);
        test_error_ret(error, "Unable to set kernel arg\n", TEST_FAIL);
        size_t gwo = 0;
        size_t gws = 1;
        size_t lws = 1;
        ...
        std::vector<cl_mem_device_address_ext> results(2, 0);
        ...
        if (results[1] != addresses[(i + 1) % numDevices])
...
REGISTER_TEST(negative_private_address)
{
    ...
    error = clSetKernelArgDevicePointerEXT(nullptr, 1, 0);
    test_failure_error_ret(
        ...
        TEST_FAIL);
    error = clSetKernelArgDevicePointerEXT(kernel, 1, 0);
    test_error_fail(
        error,
        "clSetKernelArgDevicePointerEXT failed with NULL pointer argument\n");
