...
struct MutableDispatchWorkGroups : public BasicMutableCommandBufferTest
{
    ...
    bool Skip() override
    {
        ...
        return !no_additional_wgs_support || !extension_avaliable;
    }
    ...
    cl_int SetUp(int elements) override
    {
        ...
        out_of_order_queue =
            clCreateCommandQueue(context, device, required_properties, &error);
        test_error(error, "Unable to create command queue to test with");

        command_buffer = clCreateCommandBufferKHR(
            1, &out_of_order_queue, config.command_buffer_properties, &error);
