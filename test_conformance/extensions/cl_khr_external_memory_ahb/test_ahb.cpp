...
ahb_image_size_table test_sizes[] = { { 128, 128 } };
                ...
                cl_command_queue defaultQueue, int num_elements)
{
    ...
    if (!is_extension_available(
            deviceID, "cl_khr_external_memory_android_hardware_buffer"))
    {
                ...
                if (!AHardwareBuffer_isSupported(&aHardwareBufferDesc))
                {
                    ...
                    log_info("Unsupported format %s:\n   Usage flags %s\n   "
                             ...
                             ahardwareBufferFormatToString(
                                 format.aHardwareBufferFormat),
                             usage_string, aHardwareBufferDesc.width,
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                size_t global_work_size[] = { (imageInfo.width),
                                              (imageInfo.height) };
                err = clEnqueueNDRangeKernel(defaultQueue, kernel, 2, nullptr,
                                             global_work_size, nullptr, 0,
                                             nullptr, nullptr);

                // Read buffer and verify
                std::vector<char> ocl_verify_data(verify_buffer_size);
                err = clEnqueueReadBuffer(defaultQueue, ocl_pixel_buffer,
                                          CL_BLOCKING, 0, verify_buffer_size,
                                          ocl_verify_data.data(), 0, nullptr,
                                          nullptr);
                test_error(err, "clEnqueueReadBuffer failed");

                std::vector<char> ahb_verify_data(verify_buffer_size);
                err = clEnqueueReadBuffer(defaultQueue, ahb_pixel_buffer,
                                          CL_BLOCKING, 0, verify_buffer_size,
                                          ahb_verify_data.data(), 0, nullptr,
                                          nullptr);
                test_error(err, "clEnqueueReadBuffer failed");
    ...
    for (auto format : test_formats)
    {
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                err = clEnqueueReadImage(defaultQueue, imported_image, CL_TRUE,
                                         origin, region, imageInfo.rowPitch, 0,
    ...
    for (auto format : test_formats)
    {
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                size_t origin[] = { 0, 0, 0 };
                size_t region[] = { imageInfo.width, imageInfo.height, 1 };
                err = clEnqueueCopyImage(defaultQueue, imported_image,
                                         opencl_image, origin, origin, region,
                                         0, nullptr, nullptr);
    ...
    for (auto format : test_formats)
    {
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                err = clEnqueueCopyImageToBuffer(defaultQueue, imported_image,
                ...
                err = clEnqueueReadBuffer(defaultQueue, opencl_buffer, CL_TRUE,
                                          0, srcBytes, out_buffer.data(), 0,
                                          nullptr, nullptr);
                test_error(err, "clEnqueueReadBuffer failed");
    ...
    for (auto format : test_formats)
    {
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                err = clEnqueueCopyBufferToImage(defaultQueue, opencl_buffer,
                ...
                char *out_image_ptr =
                    reinterpret_cast<char *>(hardware_buffer_data);
                char *srcData_ptr = (char *)srcData;
    ...
    for (auto format : test_formats)
    {
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                err = clEnqueueWriteImage(defaultQueue, imported_image, CL_TRUE,
    ...
    for (auto format : test_formats)
    {
        ...
        for (auto usage : test_usages)
        {
            ...
            for (auto resolution : test_sizes)
            {
                ...
                char *verificationValue =
                    static_cast<char *>(malloc(pixelSize));
                ...
                switch (format.clImageFormat.image_channel_data_type)
                {
                    case CL_HALF_FLOAT:
                        DetectFloatToHalfRoundingMode(
                            defaultQueue); // Intentional drop-through
                    case CL_UNORM_INT8: {
                        ...
                        err = clEnqueueFillImage(defaultQueue, imported_image,
