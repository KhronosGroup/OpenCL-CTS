    ...
    for(auto format : test_formats) {
        ...
        for(auto usage : test_usages) {
            ...
            for(auto resolution : test_sizes) {
                ...
                ahb_result = AHardwareBuffer_lock(aHardwareBuffer, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1, nullptr, &hardware_buffer_data);
                if(ahb_result != 0)
                {
                ...
                clMemWrapper opencl_image = clCreateImage(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, imageInfo.format, &imageDesc, srcData, &err);
                test_error(err, "Failed to create CL image");

                ExplicitTypes outputType;
                const char * readFormat;

                if(format.clImageFormat.image_channel_data_type == CL_UNSIGNED_INT8){
                    readFormat="ui";
                    outputType = kUInt;
                }
                else{
                ...
                size_t verify_buffer_size = imageInfo.width * imageInfo.height * get_explicit_type_size(outputType) * 4;

                clMemWrapper ocl_pixel_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, verify_buffer_size, nullptr, &err);
                test_error(err, "Failed to create ocl pixel buffer");

                clMemWrapper ahb_pixel_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, verify_buffer_size, nullptr, &err);
                test_error(err, "Failed to crete ahb pixel buffer");

                // Populate kernel
                std::vector<char> programSrc(2*strlen(diff_images_kernel_source));
                ...
                err = create_single_kernel_helper(context,&program, &kernel,1,&ptr,"verify_image");
                ...
                err = clSetKernelArg(kernel,2,sizeof(cl_mem),&ocl_pixel_buffer);
                test_error(err, "clSetKernelArg failed");
                ...
                for(unsigned row = 0; row < imageInfo.height; row++)
                {
                    for(unsigned col = 0; col < imageInfo.width; col++)
                    {
                        ...
                        {
                            case kFloat: {
                                auto *cl_ptr = reinterpret_cast<cl_float4*>(ocl_verify_data.data());
                                auto *ahb_ptr = reinterpret_cast<cl_float4*>(ahb_verify_data.data());

                                if((cl_ptr[pixel_index].s0 != ahb_ptr[pixel_index].s0) ||
                                    (cl_ptr[pixel_index].s1 != ahb_ptr[pixel_index].s1) ||
                                    (cl_ptr[pixel_index].s2 != ahb_ptr[pixel_index].s2) ||
                                    (cl_ptr[pixel_index].s3 != ahb_ptr[pixel_index].s3))
                                {
                                    log_error("At coord (%u, %u) expected (%f,%f,%f,%f), got (%f,%f,%f,%f)",
                                        col, row,
                                        cl_ptr[pixel_index].s0, cl_ptr[pixel_index].s1, cl_ptr[pixel_index].s2, cl_ptr[pixel_index].s3,
                                        ahb_ptr[pixel_index].s0, ahb_ptr[pixel_index].s1, ahb_ptr[pixel_index].s2, ahb_ptr[pixel_index].s3);
                                ...
                                break;
                            case kUInt: {
                                auto *cl_ptr = reinterpret_cast<cl_uint4*>(ocl_verify_data.data());
                                auto *ahb_ptr = reinterpret_cast<cl_uint4*>(ahb_verify_data.data());
                                ...
                                if((cl_ptr[pixel_index].s0 != ahb_ptr[pixel_index].s0) ||
                                    ...
                                    (cl_ptr[pixel_index].s3 != ahb_ptr[pixel_index].s3))
                                {
                                    log_error("At coord (%u, %u) expected (%u,%u,%u,%u), got (%u,%u,%u,%u)",
                                        col, row,
                                        cl_ptr[pixel_index].s0, cl_ptr[pixel_index].s1, cl_ptr[pixel_index].s2, cl_ptr[pixel_index].s3,
                                        ahb_ptr[pixel_index].s0, ahb_ptr[pixel_index].s1, ahb_ptr[pixel_index].s2, ahb_ptr[pixel_index].s3);
                                    return TEST_FAIL;
                    ...
                    if(memcmp(srcData_ptr,out_image_ptr,scanlineSize) != 0) {
                        ...
                        if (where < imageInfo.width)
                        {
                            print_first_pixel_difference_error(
                                    ...
                                    out_image_ptr + pixel_size * where, &imageInfo, line, 1);
                            return TEST_FAIL;
                        }
                    }
    ...
    for(auto format : test_formats) {
        ...
        for(auto usage : test_usages) {
            ...
            for(auto resolution : test_sizes) {
                ...
                void *hardware_buffer_data = nullptr;
                ahb_result = AHardwareBuffer_lock(aHardwareBuffer, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1, nullptr,
                                                  &hardware_buffer_data);
                if (ahb_result != 0) {
                    log_error("AHardwareBuffer_lock failed with code %d\n", ahb_result);
                    return TEST_FAIL;
                }
                ...
                clMemWrapper imported_image = clCreateImageWithProperties(context, props, CL_MEM_READ_ONLY, nullptr,
                                                                          nullptr, nullptr, &err);
                test_error(err, "Failed to create CL image from AHardwareBuffer");
                ...
                err = clSetKernelArg(kernel, 0, sizeof(cl_mem), &imported_image); /*imported image */
                test_error(err, "clSetKernelArg failed");

                err = clSetKernelArg(kernel,1,sizeof(cl_mem),&opencl_image); /*image made in opencl*/
                test_error(err, "clSetKernelArg failed");

                err = clSetKernelArg(kernel,2,sizeof(cl_mem),&ocl_pixel_buffer); /*verification buffer*/
                test_error(err, "clSetKernelArg failed");

                err = clSetKernelArg(kernel,3,sizeof(cl_mem),&ahb_pixel_buffer); /*verification buffer*/
                test_error(err, "clSetKernelArg failed");
                ...
                for(size_t line = 0; line < imageInfo.height; line ++) {
                    ...
                    if(memcmp(srcData_ptr, out_buffer_ptr, scanlineSize) != 0) {
                        ...
                        if (where < imageInfo.width)
                        {
                            print_first_pixel_difference_error(
                                    where, srcData_ptr + pixel_size * where,
                                    out_buffer_ptr + pixel_size * where, &imageInfo, line, 1);
                            return TEST_FAIL;
                        }
                    }
    ...
    for(auto format : test_formats) {
        ...
        for(auto usage : test_usages) {
            ...
            for(auto resolution : test_sizes) {
                ...
                clMemWrapper opencl_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, srcBytes, srcData, &err);
                test_error(err, "Failed to create CL buffer");
                ...
                void *hardware_buffer_data = nullptr;
                ahb_result = AHardwareBuffer_lock(aHardwareBuffer, AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN, -1, nullptr, &hardware_buffer_data);
                if(ahb_result != 0)
                {
                    log_error("AHardwareBuffer_lock failed with code %d\n", ahb_result);
                    return TEST_FAIL;
                ...
                for(size_t line = 0; line < imageInfo.height; line ++) {
                    ...
                    if(memcmp(srcData_ptr,out_image_ptr,scanlineSize) != 0) {
                        ...
                        if (where < imageInfo.width)
                        {
                            print_first_pixel_difference_error(
                                    ...
                                    out_image_ptr + pixelSize * where, &imageInfo, line, 1);
                            ahb_result = AHardwareBuffer_unlock(aHardwareBuffer, nullptr);
                            if (ahb_result != 0) {
                                log_error("AHardwareBuffer_unlock failed with code %d\n", ahb_result);
                                return TEST_FAIL;
                    ...
                    total_matched += scanlineSize;
                    srcData_ptr += scanlineSize; // image data is tightly packed in buffer
