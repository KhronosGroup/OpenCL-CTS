...
REGISTER_TEST(test_buffer_format_negative)
{
    ...
    test_assert_error(err == CL_INVALID_OPERATION, "To create a buffer the aHardwareFormat must be AHARDWAREBUFFER_FORMAT_BLOB");
...
REGISTER_TEST(test_buffer_size_negative)
{
    ...
    cl_mem buffer = clCreateBufferWithProperties(context, props, CL_MEM_READ_WRITE, buffer_size/2, nullptr, &err);
    test_assert_error(err == CL_INVALID_BUFFER_SIZE, "Wrong error value returned");
...
REGISTER_TEST(test_images_negative)
{
    ...
    cl_mem image = clCreateImageWithProperties(context, props, CL_MEM_READ_WRITE, &image_format, nullptr, nullptr, &err);
    test_assert_error(err == CL_INVALID_IMAGE_FORMAT_DESCRIPTOR, "Wrong error value returned");
    if (image != nullptr)
    {
    ...
    image = clCreateImageWithProperties(context, props, CL_MEM_READ_WRITE, nullptr, &image_desc, nullptr, &err);
    test_assert_error(err == CL_INVALID_IMAGE_DESCRIPTOR, "Wrong error value returned");
    if (image != nullptr)
    {
        test_error(clReleaseMemObject(image), "Failed to release image");
    }
