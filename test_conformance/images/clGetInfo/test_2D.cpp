                               ...
                               size_t slice_pitch)
{
    ...
    // Note: for now, just reset the pitches, since they only can actually be
    // different if we use CL_MEM_USE_HOST_PTR or CL_MEM_COPY_HOST_PTR
    ...
    switch (imageInfo->type)
    {
        case CL_MEM_OBJECT_IMAGE1D:
            if (gDebugTrace)
                log_info(" - Creating 1D image %d with flags=0x%lx "
        ...
        case CL_MEM_OBJECT_IMAGE2D:
            if (gDebugTrace)
                log_info(" - Creating 2D image %d by %d with flags=0x%lx "
                         "row_pitch=%d slice_pitch=%d host_ptr=%p...\n",
                         (int)imageInfo->width, (int)imageInfo->height,
                         (unsigned long)flags, (int)row_pitch, (int)slice_pitch,
                         host_ptr);
            break;
        case CL_MEM_OBJECT_IMAGE3D:
            imageInfo->slicePitch = imageInfo->rowPitch * imageInfo->height;
            if (gDebugTrace)
                log_info(" - Creating 3D image %d by %d by %d with flags=0x%lx "
                         "row_pitch=%d slice_pitch=%d host_ptr=%p...\n",
                         (int)imageInfo->width, (int)imageInfo->height,
                         (int)imageInfo->depth, (unsigned long)flags,
                         (int)row_pitch, (int)slice_pitch, host_ptr);
            break;
        case CL_MEM_OBJECT_IMAGE1D_ARRAY:
            imageInfo->slicePitch = imageInfo->rowPitch;
            if (gDebugTrace)
                log_info(" - Creating 1D image array %d by %d with flags=0x%lx "
                         "row_pitch=%d slice_pitch=%d host_ptr=%p...\n",
        ...
        case CL_MEM_OBJECT_IMAGE2D_ARRAY:
            ...
            if (gDebugTrace)
                log_info(
                    " - Creating 2D image array %d by %d by %d with "
                    "flags=0x%lx row_pitch=%d slice_pitch=%d host_ptr=%p...\n",
                    (int)imageInfo->width, (int)imageInfo->height,
                    (int)imageInfo->arraySize, (unsigned long)flags,
                    (int)row_pitch, (int)slice_pitch, host_ptr);
            break;
    }

    image = clCreateImage(context, flags, imageInfo->format, &imageDesc,
                          host_ptr, &error);
    if (image == NULL)
    {
        switch (imageInfo->type)
        {
            ...
            case CL_MEM_OBJECT_IMAGE2D:
                log_error(
                    "ERROR: Unable to create 2D image of size %d x %d (%s)",
                    (int)imageInfo->width, (int)imageInfo->height,
            ...
            case CL_MEM_OBJECT_IMAGE3D:
                log_error("ERROR: Unable to create 3D image of size %d x %d x "
                          "%d (%s)",
                          (int)imageInfo->width, (int)imageInfo->height,
                          (int)imageInfo->depth, IGetErrorString(error));
                break;
            case CL_MEM_OBJECT_IMAGE1D_ARRAY:
            ...
            case CL_MEM_OBJECT_IMAGE2D_ARRAY:
                log_error("ERROR: Unable to create 2D image array of size %d x "
                          "%d x %d (%s)",
                          (int)imageInfo->width, (int)imageInfo->height,
                          (int)imageInfo->arraySize, IGetErrorString(error));
                break;
        }
        return -1;
    }
    ...
    error = clGetImageInfo(image, CL_IMAGE_FORMAT, sizeof(outFormat),
                           &outFormat, NULL);
    test_error(error, "Unable to get image info (format)");
    if (outFormat.image_channel_order != imageInfo->format->image_channel_order
        || outFormat.image_channel_data_type
            != imageInfo->format->image_channel_data_type)
    {
        log_error(
            "ERROR: image format returned is invalid! (expected %s:%s, got "
            "%s:%s (%d:%d))\n",
            GetChannelOrderName(imageInfo->format->image_channel_order),
            GetChannelTypeName(imageInfo->format->image_channel_data_type),
            GetChannelOrderName(outFormat.image_channel_order),
            GetChannelTypeName(outFormat.image_channel_data_type),
            (int)outFormat.image_channel_order,
            (int)outFormat.image_channel_data_type);
        return 1;
    }

    size_t outElementSize;
    error = clGetImageInfo(image, CL_IMAGE_ELEMENT_SIZE, sizeof(outElementSize),
                           &outElementSize, NULL);
    test_error(error, "Unable to get image info (element size)");
    if (outElementSize != get_pixel_size(imageInfo->format))
    {
        log_error("ERROR: image element size returned is invalid! (expected "
                  "%d, got %d)\n",
    ...
    size_t outRowPitch;
    error = clGetImageInfo(image, CL_IMAGE_ROW_PITCH, sizeof(outRowPitch),
                           &outRowPitch, NULL);
    ...
    size_t outWidth;
    error = clGetImageInfo(image, CL_IMAGE_WIDTH, sizeof(outWidth), &outWidth,
                           NULL);
    test_error(error, "Unable to get image info (width)");
    if (outWidth != imageInfo->width)
    {
        log_error(
            "ERROR: image width returned is invalid! (expected %d, got %d)\n",
    ...
    {
        case CL_MEM_OBJECT_IMAGE1D:
        case CL_MEM_OBJECT_IMAGE1D_ARRAY: required_height = 0; break;
        case CL_MEM_OBJECT_IMAGE2D:
        case CL_MEM_OBJECT_IMAGE2D_ARRAY:
        case CL_MEM_OBJECT_IMAGE3D: required_height = imageInfo->height; break;
    }

    size_t outHeight;
    error = clGetImageInfo(image, CL_IMAGE_HEIGHT, sizeof(outHeight),
                           &outHeight, NULL);
    test_error(error, "Unable to get image info (height)");
    if (outHeight != required_height)
    {
        log_error(
            "ERROR: image height returned is invalid! (expected %d, got %d)\n",
    ...
    switch (imageInfo->type)
    {
        ...
        case CL_MEM_OBJECT_IMAGE2D:
        case CL_MEM_OBJECT_IMAGE1D_ARRAY:
        case CL_MEM_OBJECT_IMAGE2D_ARRAY: required_depth = 0; break;
        case CL_MEM_OBJECT_IMAGE3D: required_depth = imageInfo->depth; break;
    }

    size_t outDepth;
    error = clGetImageInfo(image, CL_IMAGE_DEPTH, sizeof(outDepth), &outDepth,
                           NULL);
    test_error(error, "Unable to get image info (depth)");
    if (outDepth != required_depth)
    {
        log_error(
            "ERROR: image depth returned is invalid! (expected %d, got %d)\n",
    ...
    switch (imageInfo->type)
    {
        case CL_MEM_OBJECT_IMAGE1D:
        case CL_MEM_OBJECT_IMAGE2D:
        case CL_MEM_OBJECT_IMAGE3D: required_array_size = 0; break;
        case CL_MEM_OBJECT_IMAGE1D_ARRAY:
        case CL_MEM_OBJECT_IMAGE2D_ARRAY:
    ...
    size_t outArraySize;
    error = clGetImageInfo(image, CL_IMAGE_ARRAY_SIZE, sizeof(outArraySize),
                           &outArraySize, NULL);
    test_error(error, "Unable to get image info (array size)");
    if (outArraySize != required_array_size)
    {
        log_error("ERROR: image array size returned is invalid! (expected %d, "
                  "got %d)\n",
    ...
    cl_mem outBuffer;
    error = clGetImageInfo(image, CL_IMAGE_BUFFER, sizeof(outBuffer),
                           &outBuffer, NULL);
    ...
    if (imageInfo->type == CL_MEM_OBJECT_IMAGE1D_BUFFER)
    {
        ...
        {
            log_error(
                "ERROR: cl_mem returned is invalid! (expected %p, got %p)\n",
    ...
    error = clGetImageInfo(image, CL_IMAGE_NUM_MIP_LEVELS, sizeof(numMipLevels),
                           &numMipLevels, NULL);
    test_error(error, "Unable to get image info (num mip levels)");
    if (numMipLevels != 0)
    {
        log_error("ERROR: image num_mip_levels returned is invalid! (expected "
                  "%d, got %d)\n",
    ...
    cl_uint numSamples;
    error = clGetImageInfo(image, CL_IMAGE_NUM_SAMPLES, sizeof(numSamples),
                           &numSamples, NULL);
    test_error(error, "Unable to get image info (num samples)");
    if (numSamples != 0)
    {
        log_error("ERROR: image num_samples returned is invalid! (expected %d, "
                  "got %d)\n",
                           ...
                           cl_image_format *format, cl_mem_flags flags)
{
    ...
    cl_mem_flags all_host_ptr_flags[5] = { flags, CL_MEM_ALLOC_HOST_PTR | flags,
    ...
    test_error(error,
               "Unable to get max image 2D width or max image 3D height or max "
               "memory allocation size or global memory size from device");
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                for (unsigned int j = 0;
                     j < sizeof(all_host_ptr_flags) / sizeof(cl_mem_flags); j++)
                {
                    if (gDebugTrace)
                        log_info("   at size %d,%d (flags[%u] 0x%x pitch %d)\n",
                                 (int)imageInfo.width, (int)imageInfo.height, j,
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            {
                if (gDebugTrace)
                    log_info("   at size %d,%d (flags[%u] 0x%x pitch %d) out "
                             "of %d,%d\n",
                             (int)imageInfo.width, (int)imageInfo.height, j,
                             (unsigned int)all_host_ptr_flags[j],
