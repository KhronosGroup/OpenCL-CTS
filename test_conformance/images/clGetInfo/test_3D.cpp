                           ...
                           cl_image_format *format, cl_mem_flags flags)
{
    ...
    cl_mem_flags all_host_ptr_flags[] = { flags, CL_MEM_ALLOC_HOST_PTR | flags,
    ...
    int error = clGetDeviceInfo(device, CL_DEVICE_IMAGE3D_MAX_WIDTH,
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                ...
                for (imageInfo.depth = 2; imageInfo.depth < 9;
                     imageInfo.depth++)
                {
                    for (unsigned int j = 0;
                         j < sizeof(all_host_ptr_flags) / sizeof(cl_mem_flags);
                         j++)
                    {
                        if (gDebugTrace)
                            log_info("   at size %d,%d,%d (flags[%u] 0x%lx "
                                     "pitch %d,%d)\n",
                                     (int)imageInfo.width,
                                     (int)imageInfo.height,
                                     (int)imageInfo.depth, j,
                                     (unsigned long)all_host_ptr_flags[j],
                                     (int)imageInfo.rowPitch,
                                     (int)imageInfo.slicePitch);
    ...
    else if (gTestMaxImages)
    {
        ...
        get_max_sizes(&numbeOfSizes, 100, sizes, maxWidth, maxHeight, maxDepth,
                      1, maxAllocSize, memSize, CL_MEM_OBJECT_IMAGE3D,
                      imageInfo.format);
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            {
                if (gDebugTrace)
                    log_info("   at max size %d,%d,%d (flags[%u] 0x%lx pitch "
                             "%d,%d)\n",
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                imageInfo.slicePitch =
                    imageInfo.rowPitch * (imageInfo.height + extraHeight);

                size = (cl_ulong)imageInfo.slicePitch
                    * (cl_ulong)imageInfo.depth * 4 * 4;
            ...
            {
                if (gDebugTrace)
                    log_info("   at size %d,%d,%d (flags[%u] 0x%lx pitch "
                             "%d,%d) out of %d,%d,%d\n",
                             (int)imageInfo.width, (int)imageInfo.height,
                             (int)imageInfo.depth, j,
                             (unsigned long)all_host_ptr_flags[i],
