...
int TestFunc_Half_Half_Int(const Func *f, MTdata);

#if defined( __cplusplus)
    extern "C"
#endif
const vtbl _binary_i = { "binary_i", TestFunc_Float_Float_Int, TestFunc_Double_Double_Int, TestFunc_Half_Half_Int };
...
int TestFunc_Half_Half_Int(const Func *f, MTdata d)
{
    TestInfo    test_info;
    cl_int      error;
    size_t      i, j;
    ...
    memset(&test_info, 0, sizeof(test_info));
    test_info.threadCount = GetThreadCount();
    test_info.subBufferSize = BUFFER_SIZE / (sizeof(cl_int) * RoundUpToNextPowerOfTwo(test_info.threadCount));
    test_info.scale = 1;
    if (gWimpyMode)
    {
        test_info.subBufferSize = gWimpyBufferSize / (sizeof(cl_int) * RoundUpToNextPowerOfTwo(test_info.threadCount));
        test_info.scale = (cl_uint) sizeof(cl_uint) * 2 * gWimpyReductionFactor;
    }
    test_info.step = (cl_uint)test_info.subBufferSize * test_info.scale;
    if (test_info.step / test_info.subBufferSize != test_info.scale)
    {
    ...
    if (gMeasureTimes)
    {
        ...
        cl_int *p2 = (cl_int *)gIn2;
        for (j = 0; j < BUFFER_SIZE / sizeof(cl_int); j++)
        {
            p[j] = genrand_int32(d);
            p2[j] = 3;
        }

        if ((error = clEnqueueWriteBuffer(gQueue, gInBuffer, CL_FALSE, 0, BUFFER_SIZE / (sizeof(cl_int) / sizeof(cl_half)), gIn, 0, NULL, NULL)))
        {
            vlog_error("\n*** Error %d in clEnqueueWriteBuffer ***\n", error);
            return error;
        }
        ...
        for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
        {
            size_t vectorSize = sizeof(cl_int) * sizeValues[j];
            size_t localCount = (BUFFER_SIZE + vectorSize - 1) / vectorSize;    // BUFFER_SIZE / vectorSize  rounded up
            if ((error = clSetKernelArg(test_info.k[j][0], 0, sizeof(gOutBuffer[j]), &gOutBuffer[j]))) { LogBuildError(test_info.programs[j]); goto exit; }
            if ((error = clSetKernelArg(test_info.k[j][0], 1, sizeof(gInBuffer), &gInBuffer))) { LogBuildError(test_info.programs[j]); goto exit; }
            if ((error = clSetKernelArg(test_info.k[j][0], 2, sizeof(gInBuffer2), &gInBuffer2))) { LogBuildError(test_info.programs[j]); goto exit; }
            ...
            if (gReportAverageTimes)
                bestTime = sum / PERF_LOOP_COUNT;
            double clocksPerOp = bestTime * (double)gDeviceFrequency * gComputeDevices * gSimdSize * 1e6 / (BUFFER_SIZE / sizeof(cl_int));
...
static cl_int TestHalf(cl_uint job_id, cl_uint thread_id, void *data)
{
    ...
    for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
    {
        out[j] = (cl_ushort*)clEnqueueMapBuffer(tinfo->tQueue, tinfo->outBuf[j], CL_FALSE, CL_MAP_WRITE, 0, buffer_elements * sizeof(cl_ushort), 0, NULL, e + j, &error);
        if (error || NULL == out[j])
        {
            vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j, error);
            return error;
    ...
    cl_int *p2 = (cl_int *)gIn2 + thread_id * buffer_elements;
    j = 0;
    int totalSpecialValueCount = specialValuesHalfCount * specialValuesInt3Count;
    int indx = (totalSpecialValueCount - 1) / buffer_elements;
    if (job_id <= (cl_uint)indx)
    { // test edge cases
        uint32_t x, y;
    ...
    if ((error = clEnqueueWriteBuffer(tinfo->tQueue, tinfo->inBuf, CL_FALSE, 0, buffer_elements * sizeof(cl_half), p, 0, NULL, NULL)))
    {
        vlog_error("Error: clEnqueueWriteBuffer failed! err: %d\n", error);
        goto exit;
    }

    if ((error = clEnqueueWriteBuffer(tinfo->tQueue, tinfo->inBuf2, CL_FALSE, 0, buffer_elements * sizeof(cl_int), p2, 0, NULL, NULL)))
    {
        vlog_error("Error: clEnqueueWriteBuffer failed! err: %d\n", error);
        goto exit;
    }
    ...
    for (j = 0; j < buffer_elements; j++)
    {
        ...
        r[j] = convert_float_to_half(func.f_fi(s[j], s2[j]));
    }

    // Read the data back -- no need to wait for the first N-1 buffers. This is an in order queue.
    for (j = gMinVectorSizeIndex; j + 1 < gMaxVectorSizeIndex; j++)
    {
        out[j] = (cl_ushort*)clEnqueueMapBuffer(tinfo->tQueue, tinfo->outBuf[j], CL_FALSE, CL_MAP_READ, 0, buffer_elements * sizeof(cl_ushort), 0, NULL, NULL, &error);
        if (error || NULL == out[j])
        {
            vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j, error);
            goto exit;
    ...
    // Wait for the last buffer
    out[j] = (cl_ushort*)clEnqueueMapBuffer(tinfo->tQueue, tinfo->outBuf[j], CL_TRUE, CL_MAP_READ, 0, buffer_elements * sizeof(cl_ushort), 0, NULL, NULL, &error);
    if (error || NULL == out[j])
    {
        vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j, error);
        goto exit;
            ...
            if (t[j] != q[j])
            {
                ...
                if (fail)
                {
                    vlog_error("\nERROR: %s%s: %f ulp error at {%a (0x%0.4x), %d}\nExpected: %a (half 0x%0.4x) \nActual: %a (half 0x%0.4x) at index: %d\n",
                                name, sizeNames[k], err, s[j], p[j], s2[j], convert_half_to_float(r[j]), r[j], test, q[j], (cl_uint)j);
                    error = -1;
                    goto exit;
                }
            }
