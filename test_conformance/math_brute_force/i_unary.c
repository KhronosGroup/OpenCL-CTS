...
int TestFunc_Int_Half(const Func *f, MTdata);

#if defined( __cplusplus)
    extern "C"
#endif
const vtbl _i_unary = { "i_unary", TestFunc_Int_Float, TestFunc_Int_Double, TestFunc_Int_Half };
...
int TestFunc_Int_Half(const Func *f, MTdata d)
{
    uint64_t i;
    uint32_t j, k;
    int error;
    ...
    size_t bufferSize = (gWimpyMode) ? gWimpyBufferSize : BUFFER_SIZE;
    uint64_t step = bufferSize / sizeof(cl_int);
    ...
    if (gWimpyMode)
    {
        step = (1ULL << 16) * gWimpyReductionFactor / (512);
    }
    // This test is not using ThreadPool so we need to disable FTZ here
    // for reference computations
    FPU_mode_type oldMode;
            ...
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                if (t[j] != q[j])
                {
                    ...
                    vlog_error( "\nERROR: %s%s: %d ulp error at %a (0x%0.4x): *%d vs. %d\n",
                                 f->name, sizeNames[k], err, s[j], p[j], t[j], q[j] );
                    error = -1;
                    goto exit;
                }
            }
    ...
    if (gMeasureTimes)
    {
        ...
        for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
        {
            size_t vectorSize = sizeValues[j] * sizeof(cl_uint);
            size_t localCount = (bufferSize + vectorSize - 1) / vectorSize;
            if ((error = clSetKernelArg(kernels[j], 0, sizeof(gOutBuffer[j]), &gOutBuffer[j]))) { LogBuildError(programs[j]); goto exit; }
            if ((error = clSetKernelArg(kernels[j], 1, sizeof(gInBuffer), &gInBuffer))) { LogBuildError(programs[j]); goto exit; }
            ...
            if (gReportAverageTimes)
                bestTime = sum / PERF_LOOP_COUNT;
            double clocksPerOp = bestTime * (double)gDeviceFrequency * gComputeDevices * gSimdSize * 1e6 / bufferElements;
            vlog_perf(clocksPerOp, LOWER_IS_BETTER, "clocks / element", "%sD%s", f->name, sizeNames[j]);
        }
    }
