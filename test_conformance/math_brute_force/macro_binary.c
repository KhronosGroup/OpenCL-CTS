...
int TestMacro_Int_Half_Half(const Func *f, MTdata);

#if defined( __cplusplus)
extern "C"
#endif
const vtbl _macro_binary = { "macro_binary", TestMacro_Int_Float_Float, TestMacro_Int_Double_Double, TestMacro_Int_Half_Half };
...
int TestMacro_Int_Half_Half(const Func *f, MTdata d)
{
    TestInfo    test_info;
    cl_int      error;
    size_t      i, j;
    ...
    {
        //Init input arrays
        uint16_t *p = (uint16_t *)gIn;
        uint16_t *p2 = (uint16_t *)gIn2;
...
static cl_int TestHalf(cl_uint job_id, cl_uint thread_id, void *data)
{
    ...
    for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
    {
        out[j] = (cl_short*)clEnqueueMapBuffer(tinfo->tQueue, tinfo->outBuf[j], CL_FALSE, CL_MAP_WRITE, 0, buffer_size, 0, NULL, e + j, &error);
        if (error || NULL == out[j])
        {
            vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j, error);
            return error;
    ...
    for (j = gMinVectorSizeIndex; j + 1 < gMaxVectorSizeIndex; j++)
    {
        out[j] = (cl_short*)clEnqueueMapBuffer(tinfo->tQueue, tinfo->outBuf[j], CL_FALSE, CL_MAP_READ, 0, buffer_size, 0, NULL, NULL, &error);
        if (error || NULL == out[j])
        {
            vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j, error);
            goto exit;
    ...
    // Wait for the last buffer
    out[j] = (cl_short*)clEnqueueMapBuffer(tinfo->tQueue, tinfo->outBuf[j], CL_TRUE, CL_MAP_READ, 0, buffer_size, 0, NULL, NULL, &error);
    if (error || NULL == out[j])
    {
        vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j, error);
        goto exit;
