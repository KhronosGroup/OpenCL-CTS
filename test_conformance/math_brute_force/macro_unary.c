...
int TestMacro_Int_Half(const Func *f, MTdata);

#if defined( __cplusplus)
    extern "C"
#endif
const vtbl _macro_unary = { "macro_unary", TestMacro_Int_Float, TestMacro_Int_Double, TestMacro_Int_Half };
...
int TestMacro_Int_Half(const Func *f, MTdata d)
{
    TestInfo    test_info;
    cl_int      error;
    size_t      i, j;
...
#define ref_func(s) ( signbit_test ? func.i_f_f( s ) : func.i_f( s ) ) 
    ...
    for (j = 0; j < buffer_elements; j++)
    {
        ...
        {
            if ((IsHalfSubnormal(p[j]) == 0) && !((p[j] & 0x7fffU) >= 0x7c00U) && ((p[j] & 0x7fffU) != 0x0000U)) r[j] = 1;
            else r[j] = 0;
        }
        else
