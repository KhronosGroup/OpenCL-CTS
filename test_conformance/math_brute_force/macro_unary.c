...
int TestMacro_Int_Float(const Func *f, MTdata d)
{
    ...
    test_info.scale = gWimpyMode ? (cl_uint) ((1ULL<<32)/(32*BUFFER_SIZE/sizeof(cl_float))) : 1;
    ...
    for( i = 0; i < test_info.threadCount; i++ )
    {
        ...
        for( j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++ )
        {
            ...
            {
                vlog_error( "Error: Unable to create sub-buffer of gOutBuffer for region {%ld, %ld}\n", region.origin, region.size );
                goto exit;
            }
        }
...
int TestMacro_Int_Double(const Func *f, MTdata d)
{
    ...
    test_info.scale = gWimpyMode ? (cl_uint) ((1ULL<<32)/(32*BUFFER_SIZE/sizeof(cl_double))) : 1;
    test_info.step = (cl_uint) test_info.subBufferSize * test_info.scale;
    test_info.f = f;
...
static cl_int TestDouble( cl_uint job_id, cl_uint thread_id, void *data )
{
    ...
    for( j = 0; j < buffer_elements; j++ )
    {
        ...
        if( gMinVectorSizeIndex == 0 && t[j] != q[j])
        {
            ...
            if( q[j] > t[j] )
                err = q[j] - t[j];
            vlog_error( "\nERROR: %sD: %lld ulp error at %.13la: *%lld vs. %lld\n", name,  err, ((double*) gIn)[j], t[j], q[j] );
            return -1;
        }
        ...
        for( k = MAX(1, gMinVectorSizeIndex); k < gMaxVectorSizeIndex; k++ )
        {
            ...
            if( -t[j] != q[j] )
            {
                ...
                if( q[j] > -t[j] )
                    err = q[j] + t[j];
                vlog_error( "\nERROR: %sD%s: %lld ulp error at %.13la: *%lld vs. %lld\n", name, sizeNames[k], err, ((double*) gIn)[j], -t[j], q[j] );
                return -1;
            }
        }
