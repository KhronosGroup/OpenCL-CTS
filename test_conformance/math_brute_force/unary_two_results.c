...
*/
    ...
    for( i = 0; i < (1ULL<<32); i += step )
    {
        ...
        for( j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++ )
        {
            ...
            if( (error = clEnqueueWriteBuffer(gQueue, gOutBuffer2[j], CL_FALSE, 0, BUFFER_SIZE, gOut2[j], 0, NULL, NULL)))
            {
        ...
        for( j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++ )
        {
            if( (error = clEnqueueReadBuffer(gQueue, gOutBuffer[j], CL_TRUE, 0, BUFFER_SIZE, gOut[j], 0, NULL, NULL)) )
            {
                vlog_error( "ReadArray failed %d\n", error );
                goto exit;
            }
            if( (error = clEnqueueReadBuffer(gQueue, gOutBuffer2[j], CL_TRUE, 0, BUFFER_SIZE, gOut2[j], 0, NULL, NULL)) )
            {
            ...
            for( k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++ )
            {
                ...
                if( t[j] != q[j] || t2[j] != q2[j]  )
                {
                    ...
                    if( ftz )
                    {
                        ...
                        else if( IsFloatResultSubnormal(correct2, float_ulps ) )
                        {
                        ...
                        if( IsFloatSubnormal( s[j] ) )
                        {
                            ...
                            if( IsFloatResultSubnormal( correctp, float_ulps ) || IsFloatResultSubnormal( correctn, float_ulps ) )
                            {
                                if( IsFloatResultSubnormal( correct2p, float_ulps ) || IsFloatResultSubnormal( correct2n, float_ulps ) )
                                {
                                    fail = fail && !( test == 0.0f && test2 == 0.0f);
                                    if( ! fail )
                                        err = err2 = 0.0f;
                            ...
                            else if( IsFloatResultSubnormal( correct2p, float_ulps ) || IsFloatResultSubnormal( correct2n, float_ulps ) )
                            {
                                fail = fail && ! (test2 == 0.0f && (fabsf(err) <= float_ulps));
                                if( ! fail )
                                    err2 = 0.0f;
...
int TestFunc_Double2_Double(const Func *f, MTdata d)
{
    ...
    uint64_t step = BUFFER_SIZE / sizeof( cl_double );
    int scale = (int)((1ULL<<32) / (16 * BUFFER_SIZE / sizeof( cl_double )) + 1);
    if( gWimpyMode )
        step = (1ULL<<32) / 16;
