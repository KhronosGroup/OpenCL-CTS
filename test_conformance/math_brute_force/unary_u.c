...
int TestFunc_Half_UShort(const Func *f, MTdata);

#if defined( __cplusplus)
    extern "C"
#endif
const vtbl _unary_u = { "unary_u", TestFunc_Float_UInt, TestFunc_Double_ULong, TestFunc_Half_UShort };
...
int TestFunc_Half_UShort(const Func *f, MTdata d)
{
    uint64_t i;
    uint32_t j, k;
    int error;
    ...
    for (i = 0; i < (1ULL << 32); i += step)
    {
        ...
        for (j = 0; j < bufferElements; j++)
        {
            if(!strcmp(name, "nan"))
                ...
                r[j] = convert_float_to_half(f->func.f_u( p[j] ));
        }
        // Read the data back
        for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
        {
        ...
        for (j = 0; j < bufferElements; j++)
        {
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                if (t[j] != q[j])
                {
                    double test = convert_half_to_float(q[j]);
                    double correct;
                    if(!strcmp(name, "nan"))
                        correct =convert_half_to_float(reference_nanh( p[j] ));
                    else
                    ...
                    if (fail)
                    {
                      vlog_error( "\n%s%s: %f ulp error at 0x%0.4x \nExpected: %a (0x%0.4x) \nActual: %a (0x%0.4x)\n",
                                  f->name, sizeNames[k], err, p[j], convert_half_to_float(r[j]), r[j], test, q[j] );
                        error = -1;
                        goto exit;
                    }
                }
    ...
    if (gMeasureTimes)
    {
        ...
        for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
        {
            ...
            if (gReportAverageTimes)
                bestTime = sum / PERF_LOOP_COUNT;
            double clocksPerOp = bestTime * (double)gDeviceFrequency * gComputeDevices * gSimdSize * 1e6 / (bufferSize / sizeof(cl_ushort));
