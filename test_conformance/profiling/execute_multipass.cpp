...
static const char *read3d_kernel_code =
    "\n"
    "__kernel void read3d(read_only image3d_t srcimg, __global unsigned char "
    "*dst, sampler_t sampler)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    "    int    tid_z = get_global_id(2);\n"
    "    int    indx = (tid_z * get_image_height(srcimg) + tid_y) * "
    "get_image_width(srcimg) + tid_x;\n"
    "    float4 color;\n"
    "\n"
    "    color = read_imagef(srcimg, sampler, (int4)(tid_x, tid_y, tid_z, "
    "0));\n"
    "    indx *= 4;\n"
...
static int verifyImages(cl_uchar *ptr0, cl_uchar *ptr1, cl_uchar tolerance,
                        int xsize, int ysize, int zsize, int nChannels)
{
    int x, y, z, c;
    cl_uchar *p0 = ptr0;
    cl_uchar *p1 = ptr1;
        ...
        for (y = 0; y < ysize; y++)
        {
            for (x = 0; x < xsize; x++)
            {
                for (c = 0; c < nChannels; c++)
                {
                    ...
                    {
                        log_error("  images differ at x,y,z = %d,%d,%d channel "
                                  "= %d, %d to %d\n",
...
static int run_kernel(cl_device_id device, cl_context context,
                      cl_command_queue queue, int w, int h, int d,
                      int nChannels, cl_uchar *inptr, cl_uchar *outptr)
{
    ...
    if (executeEvent)
    {
        ...
        // This synchronization point is needed in order to assume the data is
        // valid. Getting profiling information is not a synchronization point.
        ...
        err = clGetEventProfilingInfo(executeEvent, CL_PROFILING_COMMAND_START,
                                      sizeof(cl_ulong), &writeStart, NULL);
        test_error(err, "clGetEventProfilingInfo failed");

        err = clGetEventProfilingInfo(executeEvent, CL_PROFILING_COMMAND_END,
                                      sizeof(cl_ulong), &writeEnd, NULL);
        test_error(err, "clGetEventProfilingInfo failed");

        log_info("Profiling info:\n");
        log_info(
            "Time from queue to start of clEnqueueNDRangeKernel: %f seconds\n",
            (double)(writeStart - queueStart) / 1000000000000.f);
        log_info(
            "Time from start of clEnqueueNDRangeKernel to end: %f seconds\n",
...
// The main point of this test is to exercise code that causes a multipass cld
// launch for a single kernel exec at the cl level. This is done on the gpu for
// 3d launches, and it's also done to handle gdims that excede the maximums
// allowed by the hardware. In this case we use 3d to exercise the multipass
// events. In the future 3d may not be multpass, in which case we will need to
// ensure that we use gdims large enough to force multipass.
...
REGISTER_TEST(execute_multipass)
{
    ...
    {
        log_error(
            " unable to allocate %d bytes of memory for output image #1\n",
            nElements);
        free((void *)inptr);
        return -1;
    }
    ...
    if (!err)
    {
        ...
        if (err) log_error(" images do not match\n");
    }
