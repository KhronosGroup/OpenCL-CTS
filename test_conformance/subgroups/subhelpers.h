...
compare(const Ty &lhs, const Ty &rhs) {
    ...
    for (auto i = 0; i < size; ++i) {
        //log_info("Comparing vector element i: %d, r value: %d, l value: %d\n", i, rhs.s[i], lhs.s[i]);
        if (lhs.s[i] != rhs.s[i]) {
            return false;
        }
    }
...
set_value(Ty &lhs, const cl_uint &rhs) {
    ...
    for (auto i = 0; i < size; ++i) {
        //log_info("setting vector element i: %d, to value: %d\n", i, rhs);
        lhs.s[i] = rhs;
    }
    return true;
}
...
set_value(Ty &lhs, const Ty &rhs) {
   //log_info("setting vector to vector: %d, to value: %d\n", lhs, rhs);
    lhs = rhs;
    return true;
}
...
typename std::enable_if<!is_vector_type<Ty>::value, bool>::type
compare(const Ty &lhs, const Ty &rhs) {
    //log_info("Comparing scalars r value: %d, l value: %d\n", rhs, lhs);
...
typename std::enable_if<!is_vector_type<Ty>::value, bool>::type
set_value(Ty &lhs, const cl_uint &rhs) {
    //log_info("setting scalar to value: %d\n", rhs);
    return lhs = rhs;
}
...
template <typename Ty, int N=0> struct TypeName;
