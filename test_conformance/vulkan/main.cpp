...
size_t parseParams(int argc, const char *argv[], const char** argList)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0]=='-')
        {
            if (argv[i],"--debug_trace"))
            {
                debug_trace = true;
            }
            if (argv[i],"--useSingleImageKernel"))
            {
                useSingleImageKernel = true;
            }
            if (argv[i],"--useDeviceLocal"))
            {
                useDeviceLocal = true;
            }
            if (argv[i],"--disableNTHandleType"))
            {
                disableNTHandleType = true;
            }
            if (argv[i], "--enableOffset"))
            {
                enableOffset = true;
            }
            if (argv[i], "--non_dedicated"))
            {
                non_dedicated = true;
            }
            if (argv[i], "-h") == 0)
...
int main(int argc, const char *argv[])
{
    ...
    if (!(is_extension_available(devices[device_no], "cl_khr_external_memory")
	  && is_extension_available(devices[device_no], "cl_khr_external_semaphore")))
    {
