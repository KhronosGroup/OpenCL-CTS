                               ...
                               int num_elements)
{
    ...
    VulkanExternalMemoryHandleType vkExternalMemoryHandleType = getSupportedVulkanExternalMemoryHandleTypeList()[0];
    ...
    VulkanDeviceMemory* vkDeviceMem = new VulkanDeviceMemory(vkDevice, bufferSize, memoryTypeList[0], vkExternalMemoryHandleType);
    VulkanBufferList vkBufferList(1, vkDevice, bufferSize, vkExternalMemoryHandleType);
    ...
    {
        log_error("Checks failed for CL_DEVICE_EXTERNAL_MEMORY_IMPORT_HANDLE_TYPES_KHR\n");
        return TEST_FAIL;
    }
...
int test_consistency_external_image(cl_device_id deviceID,
                           cl_context _context,
                           cl_command_queue _queue,
                           int num_elements)
{
    ...
    VulkanExternalMemoryHandleType vkExternalMemoryHandleType = getSupportedVulkanExternalMemoryHandleTypeList()[0];
    VulkanImage2D* vkImage2D = new VulkanImage2D(vkDevice, VULKAN_FORMAT_R8G8B8A8_UNORM, width, height, 1, vkExternalMemoryHandleType);
    ...
    VulkanDeviceMemory* vkDeviceMem = new VulkanDeviceMemory(vkDevice, totalImageMemSize, memoryTypeList[0], vkExternalMemoryHandleType);
    ...
    case VULKAN_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_KMT:
        handle = vkDeviceMem->getHandle(vkExternalMemoryHandleType);
        errNum = check_external_memory_handle_type(devList[0], CL_EXTERNAL_MEMORY_HANDLE_OPAQUE_WIN32_KMT_KHR);
    ...
    const VkImageCreateInfo VulkanImageCreateInfo = vkImage2D->getVkImageCreateInfo();

    errNum = getCLImageInfoFromVkImageInfo(&VulkanImageCreateInfo, totalImageMemSize, &img_format, &image_desc);
    if (errNum != CL_SUCCESS)
    {
    ...
    image = clCreateImageWithProperties(context,
                                        ...
                                        NULL, // host_ptr NULL
                                        &errNum);
    test_error(errNum, "Unable to create image with NULL properties with valid image format and image desc");
    ...
    test_failure_error(errNum, CL_INVALID_IMAGE_FORMAT_DESCRIPTOR, 
                       "Image creation must fail CL_INVALID_IMAGE_FORMAT_DESCRIPTOR when image desc passed as NULL");
    ...
    test_failure_error(errNum, CL_INVALID_IMAGE_DESCRIPTOR,
                       "Image creation must fail with CL_INVALID_IMAGE_DESCRIPTOR when image desc passed as NULL");
    image.reset();
                            ...
                            int num_elements)
{
    ...
    switch (vkExternalSemaphoreHandleType) {
        ...
        case VULKAN_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_NT:
            ...
            errNum = check_external_semaphore_handle_type(devList[0], CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KHR);
            sema_props1.push_back((cl_semaphore_properties_khr)CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KHR);
            sema_props1.push_back((cl_semaphore_properties_khr)handle1);
            sema_props2.push_back((cl_semaphore_properties_khr)CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KHR);
        ...
        case VULKAN_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_KMT:
            ...
            handle1 = vkVk2Clsemaphore.getHandle(vkExternalSemaphoreHandleType);
            handle2 = vkCl2Vksemaphore.getHandle(vkExternalSemaphoreHandleType);
            errNum = check_external_semaphore_handle_type(devList[0], CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KMT_KHR);
            sema_props1.push_back((cl_semaphore_properties_khr)CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KMT_KHR);
            sema_props1.push_back((cl_semaphore_properties_khr)handle1);
    ...
    sema_props1.push_back((cl_semaphore_properties_khr)CL_DEVICE_HANDLE_LIST_KHR);
    sema_props1.push_back((cl_semaphore_properties_khr)devList[0]);
    sema_props1.push_back((cl_semaphore_properties_khr)CL_DEVICE_HANDLE_LIST_END_KHR);
    sema_props2.push_back((cl_semaphore_properties_khr)CL_DEVICE_HANDLE_LIST_KHR);
    sema_props2.push_back((cl_semaphore_properties_khr)devList[0]);
    sema_props2.push_back((cl_semaphore_properties_khr)CL_DEVICE_HANDLE_LIST_END_KHR);
    sema_props1.push_back(0);
    sema_props2.push_back(0);

    // Pass NULL properties 
    cl_semaphore_khr cl_ext_semaphore = clCreateSemaphoreWithPropertiesKHRptr(context, NULL, &errNum);
    test_failure_error(
        errNum, CL_INVALID_VALUE,
        "Semaphore creation must fail with CL_INVALID_VALUE when properties are passed as NULL");


    // Pass invalid semaphore object to wait
    errNum = clEnqueueWaitSemaphoresKHRptr(cmd_queue, 1, NULL, NULL, 0, NULL, NULL);
    test_failure_error(
        errNum, CL_INVALID_VALUE,
        "clEnqueueWaitSemaphoresKHR fails with CL_INVALID_VALUE when invalid semaphore object is passed");


    // Pass invalid semaphore object to signal 
    errNum = clEnqueueSignalSemaphoresKHRptr(cmd_queue, 1, NULL, NULL, 0, NULL, NULL);
    test_failure_error(
        errNum, CL_INVALID_VALUE,
        "clEnqueueSignalSemaphoresKHR fails with CL_INVALID_VALUE when invalid semaphore object is passed");


    // Create two semaphore objects
    clVk2Clsemaphore = clCreateSemaphoreWithPropertiesKHRptr(context, sema_props1.data(), &errNum);
    test_error(errNum, "Unable to create semaphore with valid semaphore properties");

    clCl2Vksemaphore = clCreateSemaphoreWithPropertiesKHRptr(context, sema_props2.data(), &errNum);
    test_error(errNum, "Unable to create semaphore with valid semaphore properties");

    
    // Call Signal twice consecutively 
    errNum = clEnqueueSignalSemaphoresKHRptr(cmd_queue, 1, &clVk2Clsemaphore, NULL, 0, NULL, NULL);
    test_error(errNum, "clEnqueueSignalSemaphoresKHRptr failed");

    errNum = clEnqueueSignalSemaphoresKHRptr(cmd_queue, 1, &clCl2Vksemaphore, NULL, 0, NULL, NULL);
    test_error(errNum, "clEnqueueSignalSemaphoresKHRptr failed for two consecutive wait events");


    // Call Wait twice consecutively 
    errNum = clEnqueueWaitSemaphoresKHRptr(cmd_queue, 1, &clVk2Clsemaphore, NULL, 0, NULL, NULL);
    test_error(errNum, "clEnqueueWaitSemaphoresKHRptr failed");

    errNum = clEnqueueWaitSemaphoresKHRptr(cmd_queue, 1, &clCl2Vksemaphore, NULL, 0, NULL, NULL);
    test_error(errNum, "clEnqueueWaitSemaphoresKHRptr failed for two consecutive wait events");
    ...
    test_failure_error(
        errNum, CL_INVALID_VALUE,
        "clReleaseSemaphoreObjectKHRptr fails with CL_INVALID_VALUE when NULL semaphore object is passed");
