...
const cl_kernel getKernelType(VulkanFormat format, cl_kernel kernel_float, cl_kernel kernel_signed, cl_kernel kernel_unsigned)
{
    cl_kernel kernel;
    switch (format) {
...
int run_test_with_two_queue(cl_context& context, cl_command_queue& cmd_queue1,cl_command_queue& cmd_queue2, cl_kernel* kernel_unsigned,cl_kernel* kernel_signed,cl_kernel* kernel_float, VulkanDevice& vkDevice)
{
    ...
    uint64_t maxImage2DSize = MAX_2D_IMAGE_WIDTH * MAX_2D_IMAGE_HEIGHT * MAX_2D_IMAGE_ELEMENT_SIZE * 2;
    VulkanBuffer vkSrcBuffer(vkDevice, maxImage2DSize);
    VulkanDeviceMemory vkSrcBufferDeviceMemory(vkDevice, vkSrcBuffer.getSize(), getVulkanMemoryType(vkDevice, VULKAN_MEMORY_TYPE_PROPERTY_HOST_VISIBLE_COHERENT));
    ...
    VulkanDescriptorSetLayoutBindingList vkDescriptorSetLayoutBindingList(VULKAN_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VULKAN_DESCRIPTOR_TYPE_STORAGE_IMAGE, MAX_2D_IMAGE_DESCRIPTORS);
    ...
    for (size_t fIdx = 0; fIdx < vkFormatList.size(); fIdx++) {
        ...
        patternToSubstituteMap[GLSL_FORMAT_STRING] = getVulkanFormatGLSLFormat(vkFormat);
        patternToSubstituteMap[GLSL_TYPE_PREFIX_STRING] = getVulkanFormatGLSLTypePrefix(vkFormat);

        VulkanShaderModule vkImage2DShaderModule(vkDevice, prepareVulkanShader(vkImage2DShader, patternToSubstituteMap));
        VulkanComputePipeline vkComputePipeline(vkDevice, vkPipelineLayout, vkImage2DShaderModule);
        ...
        for (size_t wIdx = 0; wIdx < ARRAY_SIZE(widthList); wIdx++) {
            ...
            for (size_t hIdx = 0; hIdx < ARRAY_SIZE(heightList); hIdx++) {
                ...
                magicValue++;
                char *vkSrcBufferDeviceMemoryPtr = (char *)vkSrcBufferDeviceMemory.map();
                uint64_t srcBufSize = 0;
                memset(vkSrcBufferDeviceMemoryPtr, 0, maxImage2DSize);
                memset(srcBufferPtr, 0, maxImage2DSize);
                uint32_t mipLevel = 0;
                for (uint32_t row = 0; row < std::max(height >> mipLevel, uint32_t(1)); row++) {
                    for (uint32_t col = 0; col < std::max(width >> mipLevel, uint32_t(1)); col++) {
                        for (uint32_t elementByte = 0; elementByte < elementSize; elementByte++) {
                            vkSrcBufferDeviceMemoryPtr[srcBufSize] = (char)(magicValue + mipLevel + row + col);
                            srcBufferPtr[srcBufSize] = (char)(magicValue + mipLevel + row + col);
                            srcBufSize++;
                        }
                    }
                }
                srcBufSize = ROUND_UP(srcBufSize, std::max(elementSize, (uint32_t)VULKAN_MIN_BUFFER_OFFSET_COPY_ALIGNMENT));
                vkSrcBufferDeviceMemory.unmap();

                for (size_t niIdx = 0; niIdx < ARRAY_SIZE(num2DImagesList); niIdx++) {
                    uint32_t num2DImages = num2DImagesList[niIdx] + 1; //added one image for cross-cq case for updateKernelCQ2
                    ...
                    for (size_t emhtIdx = 0; emhtIdx < vkExternalMemoryHandleTypeList.size(); emhtIdx++) {
                        ...
                        if ((true == disableNTHandleType) && (VULKAN_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_NT == vkExternalMemoryHandleType)) {
                            // Skip running for WIN32 NT handle.
                            continue;
                        }
                        VulkanImage2D vkDummyImage2D(vkDevice, vkFormatList[0], widthList[0], heightList[0], 1, vkExternalMemoryHandleType);
                        const VulkanMemoryTypeList & memoryTypeList = vkDummyImage2D.getMemoryTypeList();
                        ...
                        for (size_t mtIdx = 0; mtIdx < memoryTypeList.size(); mtIdx++) {
                            ...
                            {
                                VulkanImage2D vkImage2D(vkDevice, vkFormat, width, height, numMipLevels, vkExternalMemoryHandleType);
                            ...
                            VulkanImage2DList vkNonDedicatedImage2DList(num2DImages, vkNonDedicatedImage2DListDeviceMemory1, 0, 0/*interImageOffset*/, vkDevice, vkFormat, width, height, numMipLevels, vkExternalMemoryHandleType);
                            for (size_t bIdx = 0; bIdx < num2DImages; bIdx++) {
                                nonDedicatedExternalMemory1.push_back(new clExternalMemoryImage(*vkNonDedicatedImage2DListDeviceMemory1[bIdx], vkExternalMemoryHandleType, context, totalImageMemSize, width, height, 0, vkNonDedicatedImage2DList[bIdx], deviceId));
                            }
                            VulkanImageViewList vkNonDedicatedImage2DViewList(vkDevice, vkNonDedicatedImage2DList);
                            VulkanImage2DList vkNonDedicatedImage2DList2(num2DImages, vkNonDedicatedImage2DListDeviceMemory2, 0, 0/*interImageOffset*/, vkDevice, vkFormat, width, height, numMipLevels, vkExternalMemoryHandleType);
                            for (size_t bIdx = 0; bIdx < num2DImages; bIdx++) {
                                nonDedicatedExternalMemory2.push_back(new clExternalMemoryImage(*vkNonDedicatedImage2DListDeviceMemory2[bIdx], vkExternalMemoryHandleType, context, totalImageMemSize, width, height, 0, vkNonDedicatedImage2DList2[bIdx], deviceId));
                            }
                            ...
                            VulkanImage2DList & vkImage2DList = vkNonDedicatedImage2DList;
                            VulkanImageViewList & vkImage2DViewList = vkNonDedicatedImage2DViewList;

                            clCl2VkExternalSemaphore->signal(cmd_queue1);
                            if (!useSingleImageKernel) {
                                for (size_t i2DIdx = 0; i2DIdx < vkImage2DList.size(); i2DIdx++) {
                                    for (uint32_t mipLevel = 0; mipLevel < numMipLevels; mipLevel++) {
                                        uint32_t i2DvIdx = (uint32_t)(i2DIdx * numMipLevels) + mipLevel;
                                        vkDescriptorSet.update(1 + i2DvIdx, vkImage2DViewList[i2DvIdx]);
                                    }
                                }
                                vkCopyCommandBuffer.begin();
                                vkCopyCommandBuffer.pipelineBarrier(vkImage2DList, VULKAN_IMAGE_LAYOUT_UNDEFINED, VULKAN_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
                                for (size_t i2DIdx = 0; i2DIdx < vkImage2DList.size(); i2DIdx++) {
                                    vkCopyCommandBuffer.copyBufferToImage(vkSrcBuffer, vkImage2DList[i2DIdx], VULKAN_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
                                }
                                vkCopyCommandBuffer.pipelineBarrier(vkImage2DList, VULKAN_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VULKAN_IMAGE_LAYOUT_GENERAL);
                                vkCopyCommandBuffer.end();
                                memset(dstBufferPtr, 0, srcBufSize);
                                vkQueue.submit(vkCopyCommandBuffer);
                                vkShaderCommandBuffer.begin();
                                vkShaderCommandBuffer.bindPipeline(vkComputePipeline);
                                vkShaderCommandBuffer.bindDescriptorSets(vkComputePipeline, vkPipelineLayout, vkDescriptorSet);
                                vkShaderCommandBuffer.dispatch(NUM_BLOCKS(width, NUM_THREADS_PER_GROUP_X), NUM_BLOCKS(height, NUM_THREADS_PER_GROUP_Y / 2), 1);
                                vkShaderCommandBuffer.end();
                            }
                            for (uint32_t iter = 0; iter < innerIterations; iter++) {
                                if (useSingleImageKernel) {
                                    for (size_t i2DIdx = 0; i2DIdx < vkImage2DList.size(); i2DIdx++) { 
                                ...
                                vkQueue.submit(vkCl2VkSemaphore, vkShaderCommandBuffer, vkVk2CLSemaphore);
                                clVk2CLExternalSemaphore->wait(cmd_queue1);
                                switch (num2DImages) {
                                    case 2:
                                        updateKernelCQ1 = getKernelType(vkFormat, kernel_float[0], kernel_signed[0], kernel_unsigned[0]);
                                        break;
                                    case 3:
                                        updateKernelCQ1 = getKernelType(vkFormat, kernel_float[1], kernel_signed[1], kernel_unsigned[1]);
                                        break;
                                    case 5:
                                        updateKernelCQ1 = getKernelType(vkFormat, kernel_float[2], kernel_signed[2], kernel_unsigned[2]);
                                        break;
                                }
                                updateKernelCQ2 = getKernelType(vkFormat, kernel_float[3], kernel_signed[3], kernel_unsigned[3]); //similar kernel-type based on vkFormat
                                int j = 0;
                                // Setting arguments of updateKernelCQ2

                                err =  clSetKernelArg(updateKernelCQ2, 0, sizeof(cl_mem), &external_mem_image1[0]);
                                err |= clSetKernelArg(updateKernelCQ2, 1, sizeof(cl_mem), &external_mem_image2[0]);
                                err |= clSetKernelArg(updateKernelCQ2, 2, sizeof(cl_mem), &external_mem_image1[num2DImages - 1]);
                                err |= clSetKernelArg(updateKernelCQ2, 3, sizeof(cl_mem), &external_mem_image2[num2DImages - 1]);
                                err |= clSetKernelArg(updateKernelCQ2, 4, sizeof(unsigned int), &num2DImages);
                                err |= clSetKernelArg(updateKernelCQ2, 5, sizeof(unsigned int), &width);
                                err |= clSetKernelArg(updateKernelCQ2, 6, sizeof(unsigned int), &height);
                                err |= clSetKernelArg(updateKernelCQ2, 7, sizeof(unsigned int), &numMipLevels);
                                ...
                                err |= clSetKernelArg(updateKernelCQ1, j, sizeof(unsigned int), &num2DImages);
                                err |= clSetKernelArg(updateKernelCQ1, ++j, sizeof(unsigned int), &width);
                                err |= clSetKernelArg(updateKernelCQ1, ++j, sizeof(unsigned int), &height);
                                err |= clSetKernelArg(updateKernelCQ1, ++j, sizeof(unsigned int), &numMipLevels);

                                if (err != CL_SUCCESS)
                                {
                                    print_error(err, "Error: Failed to set arg values \n");
                                    goto CLEANUP;
                                }
                                //clVk2CLExternalSemaphore->wait(cmd_queue1);
                            ...
                            for (int i=0;i<num2DImages;i++){
                                delete vkNonDedicatedImage2DListDeviceMemory1[i];
                                delete vkNonDedicatedImage2DListDeviceMemory2[i];
                                delete nonDedicatedExternalMemory1[i];
                                delete nonDedicatedExternalMemory2[i];
                            }
                            vkNonDedicatedImage2DListDeviceMemory1.erase(vkNonDedicatedImage2DListDeviceMemory1.begin(),vkNonDedicatedImage2DListDeviceMemory1.begin()+num2DImages);
...
int run_test_with_one_queue(cl_context& context, cl_command_queue& cmd_queue1, cl_kernel* kernel_unsigned,cl_kernel* kernel_signed,cl_kernel* kernel_float, VulkanDevice& vkDevice)
{
    cl_int err = CL_SUCCESS;
    size_t origin[3] = {0, 0, 0};
    size_t region[3] = {1, 1, 1};
                ...
                for (size_t niIdx = 0; niIdx < ARRAY_SIZE(num2DImagesList); niIdx++) {
                    ...
                    for (size_t emhtIdx = 0; emhtIdx < vkExternalMemoryHandleTypeList.size(); emhtIdx++) {
                        ...
                        for (size_t mtIdx = 0; mtIdx < memoryTypeList.size(); mtIdx++) {
                            ...
                            for (size_t bIdx = 0; bIdx < vkNonDedicatedImage2DList.size(); bIdx++) {
                               // Create list of Vulkan device memories and bind the list of Vulkan images. 
                                vkNonDedicatedImage2DListDeviceMemory1.push_back(new VulkanDeviceMemory(vkDevice, totalImageMemSize, memoryType, vkExternalMemoryHandleType));
                                vkNonDedicatedImage2DListDeviceMemory1[bIdx]->bindImage(vkNonDedicatedImage2DList[bIdx], 0);
                                nonDedicatedExternalMemory1.push_back(new clExternalMemoryImage(*vkNonDedicatedImage2DListDeviceMemory1[bIdx], vkExternalMemoryHandleType, context, totalImageMemSize, width, height, 0, vkNonDedicatedImage2DList[bIdx], deviceId));
                            }
                            VulkanImageViewList vkNonDedicatedImage2DViewList(vkDevice, vkNonDedicatedImage2DList);
                            ...
                            for (size_t bIdx = 0; bIdx < vkNonDedicatedImage2DList2.size(); bIdx++) {
                                vkNonDedicatedImage2DListDeviceMemory2.push_back(new VulkanDeviceMemory(vkDevice, totalImageMemSize, memoryType, vkExternalMemoryHandleType));
                                vkNonDedicatedImage2DListDeviceMemory2[bIdx]->bindImage(vkNonDedicatedImage2DList2[bIdx], 0);
                                nonDedicatedExternalMemory2.push_back(new clExternalMemoryImage(*vkNonDedicatedImage2DListDeviceMemory2[bIdx], vkExternalMemoryHandleType, context, totalImageMemSize, width, height, 0, vkNonDedicatedImage2DList2[bIdx], deviceId));
                            }
                            VulkanImageViewList vkDedicatedImage2DViewList(vkDevice, vkNonDedicatedImage2DList2);
                            ...
                            for (uint32_t iter = 0; iter < innerIterations; iter++) {
                                ...
                                if (err != CL_SUCCESS)
                                {
                                    print_error(err, "Error: Failed to set arg values for kernel-1\n");
                                    goto CLEANUP;
                                }
...
int test_image_common(cl_device_id device_, cl_context context_, cl_command_queue queue_, int numElements_)
{
    ...
    const uint32_t num_kernels = ARRAY_SIZE(num2DImagesList) + 1; // One kernel for Cross-CQ case
    const uint32_t num_kernel_types = 3;
    const char *kernel_source[num_kernels] = {kernel_text_numImage_1, kernel_text_numImage_2, kernel_text_numImage_4};
    char source_1[4096];
    char source_2[4096];
    char source_3[4096];
    ...
    for (int i = 0; i<num_kernels; i++) {
        switch(i) {
            case 0:
                ...
                sprintf(source_2, kernel_source[i], "int4", "i", "int4", "i", "i", "i");
                sprintf(source_3, kernel_source[i], "uint4", "ui", "uint4", "ui", "ui", "ui");
                break;
            case 1:
                sprintf(source_1, kernel_source[i], "float4", "f", "float4", "f","float4", "f","float4", "f", "f", "f", "f", "f");
                sprintf(source_2, kernel_source[i], "int4", "i", "int4", "i", "int4", "i", "int4", "i", "i", "i", "i", "i");
                sprintf(source_3, kernel_source[i], "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "ui", "ui", "ui", "ui");
                break;
            case 2:
                sprintf(source_1, kernel_source[i], "float4", "f", "float4", "f", "float4", "f", "float4", "f", "float4", "f", "float4", "f", "float4", "f", "float4", "f", "f", "f", "f", "f", "f", "f", "f", "f");
                sprintf(source_2, kernel_source[i], "int4", "i", "int4", "i", "int4", "i", "int4", "i", "int4", "i", "int4", "i", "int4", "i", "int4", "i", "i", "i", "i", "i", "i", "i", "i", "i");
                sprintf(source_3, kernel_source[i], "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "ui", "ui", "ui", "ui", "ui", "ui", "ui", "ui");
                break;
            case 3: // Addtional case for creating updateKernelCQ2 which takes two images
                sprintf(source_1, kernel_source[1], "float4", "f", "float4", "f","float4", "f","float4", "f", "f", "f", "f", "f");
                sprintf(source_2, kernel_source[1], "int4", "i", "int4", "i", "int4", "i", "int4", "i", "i", "i", "i", "i");
                sprintf(source_3, kernel_source[1], "uint4", "ui", "uint4", "ui", "uint4", "ui", "uint4", "ui", "ui", "ui", "ui", "ui");
                break;
        }
        const char *sourceTexts[num_kernel_types] = {source_1, source_2, source_3};
