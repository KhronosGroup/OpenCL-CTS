...
void init_cl_vk_ext(cl_platform_id opencl_platform){
    ...
    if (NULL == clCreateSemaphoreWithPropertiesKHRptr) {
        throw std::runtime_error("Failed to get the function pointer of clCreateSemaphoreWithPropertiesKHRptr!");
    }
}
...
cl_int get2DImageDimensions(const VkImageCreateInfo* VulkanImageCreateInfo, cl_image_format* img_fmt, size_t totalImageSize, size_t &width, size_t &height )
{
    cl_int result = CL_SUCCESS;
    if (totalImageSize == 0) {
        result = CL_INVALID_VALUE;
...
cl_int getCLImageInfoFromVkImageInfo(const VkImageCreateInfo *VulkanImageCreateInfo, size_t totalImageSize, cl_image_format *img_fmt, cl_image_desc *img_desc)
{
    cl_int result = CL_SUCCESS;
    ...
    result = get2DImageDimensions(VulkanImageCreateInfo, img_fmt, totalImageSize, img_desc->image_width, img_desc->image_height);
    if ( CL_SUCCESS != result) {
...
cl_int check_external_memory_handle_type(cl_device_id deviceID, cl_external_mem_handle_type_khr requiredHandleType)
{
    ...
    test_error(errNum, " Unable to query CL_DEVICE_EXTERNAL_MEMORY_IMPORT_HANDLE_TYPES_KHR \n");
...
cl_int check_external_semaphore_handle_type(cl_device_id deviceID, cl_external_semaphore_handle_type_khr requiredHandleType)
{
    ...
    test_error(errNum, " Unable to query CL_DEVICE_SEMAPHORE_IMPORT_HANDLE_TYPES_KHR \n");
...
clExternalMemory::clExternalMemory(
    const clExternalMemory & externalMemory) : m_externalMemory(externalMemory.m_externalMemory) {	
}
    ...
    cl_device_id deviceId)
{
    ...
    const VkImageCreateInfo VulkanImageCreateInfo = image2D.getVkImageCreateInfo();
    
    errcode_ret = getCLImageInfoFromVkImageInfo( &VulkanImageCreateInfo, image2D.getSize(), &img_format, &image_desc);
    if (CL_SUCCESS != errcode_ret) {
...
cl_mem clExternalMemory::getExternalMemoryBuffer()
{
...
clExternalSemaphore::clExternalSemaphore(
    const clExternalSemaphore & externalSemaphore) : m_externalSemaphore(externalSemaphore.m_externalSemaphore) {
...
clExternalSemaphore::clExternalSemaphore(
    ...
    switch (externalSemaphoreHandleType) {
     ...
     case VULKAN_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_NT:
         ...
         #endif
         log_info(" Opaque NT handles are only supported on Windows\n");
         handle =  semaphore.getName().size() ? NULL : semaphore.getHandle(externalSemaphoreHandleType);
         err = check_external_semaphore_handle_type(devList[0], CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KHR);
         sema_props.push_back((cl_semaphore_properties_khr)CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KHR);
     ...
     case VULKAN_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_KMT:
         ...
         err = check_external_semaphore_handle_type(devList[0], CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KMT_KHR);
         sema_props.push_back((cl_semaphore_properties_khr)CL_SEMAPHORE_HANDLE_OPAQUE_WIN32_KMT_KHR);
         sema_props.push_back((cl_semaphore_properties_khr)handle);
         break;
     default:
        ASSERT(0);
    ...
    m_externalSemaphore = clCreateSemaphoreWithPropertiesKHRptr(context, sema_props.data(), &err);
    if (CL_SUCCESS != err) {
            log_error("clCreateSemaphoreWithPropertiesKHRptr failed with %d\n", err);
            throw std::runtime_error("clCreateSemaphoreWithPropertiesKHRptr failed! ");
    }
}
...
void clExternalSemaphore::signal(cl_command_queue cmd_queue) {
    clEnqueueSignalSemaphoresKHRptr(cmd_queue, 1, &m_externalSemaphore, NULL, 0, NULL, NULL);
}

void clExternalSemaphore::wait( cl_command_queue cmd_queue) {
    clEnqueueWaitSemaphoresKHRptr(cmd_queue, 1, &m_externalSemaphore, NULL, 0, NULL, NULL);
}
