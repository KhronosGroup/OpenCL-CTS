    ...
    uint32_t numQueuesPerFamily)
{
    ...
    const VulkanPhysicalDeviceList & physicalDeviceList = getVulkanInstance().getPhysicalDeviceList();
    for (size_t pdIdx = 0; pdIdx < physicalDeviceList.size(); pdIdx++) {
        maxQueueFamilyCount = std::max(maxQueueFamilyCount, (uint32_t)physicalDeviceList[pdIdx].getQueueFamilyList().size());
    }
...
void
VulkanQueueFamilyToQueueListMap::insert(
    uint32_t key,
    VulkanQueueList & queueList)
{
    m_map.insert(std::pair<uint32_t, std::reference_wrapper<VulkanQueueList> >(key, std::reference_wrapper<VulkanQueueList>(queueList)));
}

VulkanQueueList &
VulkanQueueFamilyToQueueListMap::operator[](
    uint32_t key)
{
...
VulkanQueueList::VulkanQueueList(
    const VulkanQueueList & queueList)
{
    ...
    VulkanShaderStage shaderStage)
{
    for (size_t idx = 0; idx < numDescriptorSetLayoutBindings; idx++) {
        VulkanDescriptorSetLayoutBinding *descriptorSetLayoutBinding = new VulkanDescriptorSetLayoutBinding((uint32_t)idx, descriptorType, descriptorCount, shaderStage);
        add(*descriptorSetLayoutBinding);
    }
}

VulkanDescriptorSetLayoutBindingList::VulkanDescriptorSetLayoutBindingList(
    VulkanDescriptorType descriptorType0,
    uint32_t descriptorCount0,
    VulkanDescriptorType descriptorType1,
    uint32_t descriptorCount1,
    VulkanShaderStage shaderStage)
{
    for (uint32_t idx = 0; idx < descriptorCount0; idx++) {
        VulkanDescriptorSetLayoutBinding *descriptorSetLayoutBinding0 = new VulkanDescriptorSetLayoutBinding(idx, descriptorType0, 1, shaderStage);
        add(*descriptorSetLayoutBinding0);
    }
    for (uint32_t idx = 0; idx < descriptorCount1; idx++) {
        VulkanDescriptorSetLayoutBinding *descriptorSetLayoutBinding1 = new VulkanDescriptorSetLayoutBinding(descriptorCount0 + idx, descriptorType1, 1, shaderStage);
...
VulkanDescriptorSetLayoutBindingList::~VulkanDescriptorSetLayoutBindingList()
{
    for (size_t idx = 0; idx < m_wrapperList.size(); idx++) {
        VulkanDescriptorSetLayoutBinding & descriptorSetLayoutBinding = m_wrapperList[idx];
    ...
    const VulkanCommandPool & commandPool)
{
    for (size_t idx = 0; idx < numCommandBuffers; idx++) {
        VulkanCommandBuffer * commandBuffer = new VulkanCommandBuffer(device, commandPool);
        add(*commandBuffer);
    }
}
...
VulkanBufferList::VulkanBufferList(
    const VulkanBufferList & bufferList)
{
    ...
    const VulkanQueueFamilyList & queueFamilyList)
{
    for (size_t bIdx = 0; bIdx < numBuffers; bIdx++) {
        VulkanBuffer *buffer = new VulkanBuffer(device, size, externalMemoryHandleType, bufferUsage, sharingMode, queueFamilyList);
        add(*buffer);
    }
}
...
VulkanImage2DList::VulkanImage2DList(
    size_t numImages,
    std::vector<VulkanDeviceMemory *>& deviceMemory,
    uint64_t baseOffset,
    uint64_t interImageOffset,
    const VulkanDevice & device,
    VulkanFormat format,
    uint32_t width,
    uint32_t height,
    ...
    VulkanSharingMode sharingMode)
{
    for (size_t i2DIdx = 0; i2DIdx < numImages; i2DIdx++) {
        VulkanImage2D *image2D = new VulkanImage2D(device, format, width, height, mipLevels, externalMemoryHandleType, imageCreateFlag, imageUsage, sharingMode);
        add(*image2D);
        deviceMemory[i2DIdx]->bindImage(*image2D, baseOffset + (i2DIdx * interImageOffset));
    }
}
...
VulkanImageViewList::VulkanImageViewList(
    const VulkanImageViewList & image2DList)
{
    ...
    bool createImageViewPerMipLevel)
{
    for (size_t i2DIdx = 0; i2DIdx < image2DList.size(); i2DIdx++) {
        if (createImageViewPerMipLevel) {
            for (uint32_t mipLevel = 0; mipLevel < image2DList[i2DIdx].getNumMipLevels(); mipLevel++) {
                VulkanImageView *image2DView = new VulkanImageView(device, image2DList[i2DIdx], VULKAN_IMAGE_VIEW_TYPE_2D, mipLevel, 1);
                add(*image2DView);
            }
        }
        else {
            VulkanImageView *image2DView = new VulkanImageView(device, image2DList[i2DIdx], VULKAN_IMAGE_VIEW_TYPE_2D);
            add(*image2DView);
        }
    }
}
...
{
    for (size_t i2DIdx = 0; i2DIdx < image2DList.size(); i2DIdx++) {
        VulkanDeviceMemory *deviceMemory = new VulkanDeviceMemory(device, image2DList[i2DIdx], memoryType, externalMemoryHandleType);
        add(*deviceMemory);
    ...
    const std::wstring namePrefix)
{
    ...
    for (size_t idx = 0; idx < numSemaphores; idx++) {
        if (namePrefix.size()) {
            ...
            swprintf(tempName, maxNameSize, L"%s%d", namePrefix.c_str(), (int)idx);
            name = tempName;
        }
        VulkanSemaphore * semaphore = new VulkanSemaphore(device, externalSemaphoreHandleType, name);
        add(*semaphore);
    }
}
