...
public:
    virtual void add(
        const VulkanWrapper & wrapper);
    virtual size_t size() const;
    virtual const VulkanWrapper & operator[](
        size_t idx) const;
    virtual VulkanWrapper & operator[](
        size_t idx);
...
public:
    void insert(
        const VulkanKey & key,
        VulkanValue & value);
    const VulkanValue & operator[](
        const VulkanKey & key) const;
    VulkanValue & operator[](
        const VulkanKey & key);
};

class VulkanPhysicalDeviceList : public VulkanList<VulkanPhysicalDevice, VkPhysicalDevice>
{
...
class VulkanQueueFamilyToQueueListMap : public VulkanMap<uint32_t, std::reference_wrapper<VulkanQueueList> >
{
protected:
...
class VulkanDescriptorSetLayoutBindingList : public VulkanList<VulkanDescriptorSetLayoutBinding, VkDescriptorSetLayoutBinding>
{
protected:
...
class VulkanDescriptorSetLayoutList : public VulkanList<VulkanDescriptorSetLayout, VkDescriptorSetLayout>
{
protected:
...
class VulkanCommandBufferList : public VulkanList<VulkanCommandBuffer, VkCommandBuffer>
{
protected:
...
public:
    VulkanBufferList(
        ...
        const VulkanDevice & device,
        uint64_t size,
        VulkanExternalMemoryHandleType externalMemoryHandleType = VULKAN_EXTERNAL_MEMORY_HANDLE_TYPE_NONE,
        VulkanBufferUsage bufferUsage = VULKAN_BUFFER_USAGE_STORAGE_BUFFER_TRANSFER_SRC_DST,
        VulkanSharingMode sharingMode = VULKAN_SHARING_MODE_EXCLUSIVE,
        const VulkanQueueFamilyList & queueFamilyList = getEmptyVulkanQueueFamilyList());
...
public:
    VulkanImage2DList(
        ...
        VulkanImageUsage imageUsage = VULKAN_IMAGE_USAGE_SAMPLED_STORAGE_TRANSFER_SRC_DST,
        VulkanSharingMode sharingMode = VULKAN_SHARING_MODE_EXCLUSIVE);
    VulkanImage2DList(
        size_t numImages,
        const VulkanDevice & device,
...
class VulkanDeviceMemoryList : public VulkanList<VulkanDeviceMemory, VkDeviceMemory>
{
protected:
...
public:
    ...
    VulkanSemaphoreList(
        ...
        const VulkanDevice & device,
        VulkanExternalSemaphoreHandleType externalSemaphoreHandleType = VULKAN_EXTERNAL_SEMAPHORE_HANDLE_TYPE_NONE,
...
template <class VulkanWrapper, class VulkanNative>
void
VulkanList<VulkanWrapper, VulkanNative>::add(
    VulkanWrapper & wrapper)
{

    if (m_constWrapperList.size() != size_t(0)) {
        std::cout<<"This list can only contain externally allocated objects"<<std::endl;
        return;
    }
    ...
    const VulkanWrapper & wrapper)
{
    if (m_wrapperList.size() != size_t(0)) {
        std::cout<<"This list cannot contain externally allocated objects"<<std::endl;
        return;
    }

    m_constWrapperList.push_back(std::reference_wrapper<const VulkanWrapper>(wrapper));
    m_nativeList.push_back((VulkanNative)wrapper);
}

template <class VulkanWrapper, class VulkanNative>
size_t
VulkanList<VulkanWrapper, VulkanNative>::size() const
{
    return (m_wrapperList.size() > 0) ? m_wrapperList.size() : m_constWrapperList.size();
}

template <class VulkanWrapper, class VulkanNative>
const VulkanWrapper &
VulkanList<VulkanWrapper, VulkanNative>::operator[](
    size_t idx) const
{
    if (idx < size()) {
    //CHECK_LT(idx, size());
        return (m_wrapperList.size() > 0) ? m_wrapperList[idx].get() : m_constWrapperList[idx].get();
    }
}
...
template <class VulkanKey, class VulkanValue>
void
VulkanMap<VulkanKey, VulkanValue>::insert(
    const VulkanKey & key,
    VulkanValue & value)
{
    m_map.insert(std::pair<VulkanKey, std::reference_wrapper<VulkanValue> >(key, std::reference_wrapper<VulkanValue>(value)));
}

template <class VulkanKey, class VulkanValue>
const VulkanValue &
VulkanMap<VulkanKey, VulkanValue>::operator[](
    const VulkanKey & key) const
{
    return m_map.at(key);
}
