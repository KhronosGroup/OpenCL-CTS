...
VulkanInstance::VulkanInstance(): m_vkInstance(VK_NULL_HANDLE)
{
    ...
    vkStatus = vkEnumerateInstanceExtensionProperties(NULL, &instanceExtensionPropertiesCount, NULL);
    // Something went wrong in vulkan initialization (most likely incompatible device/driver combination)
    if(vkStatus == VK_ERROR_INCOMPATIBLE_DRIVER) {
         throw std::runtime_error("Waiving vulkan test because vkEnumerateInstanceExtensionProperties failed.");
         //return WAIVED;
    }
    ...
    #define VK_FUNC_DECL(name) \
        ...
        //ASSERT_NEQ((unsigned long long)name, 0ULL) << "Couldn't obtain address for function" << #name;
...
const VulkanPhysicalDeviceList &
VulkanInstance::getPhysicalDeviceList() const
{
...
VulkanPhysicalDevice::VulkanPhysicalDevice(
    ...
    : m_vkPhysicalDevice(physicalDevice.m_vkPhysicalDevice),
      ...
      m_vkPhysicalDeviceMemoryProperties(physicalDevice.m_vkPhysicalDeviceMemoryProperties),
    ...
    : m_vkPhysicalDevice(vkPhysicalDevice)
{
    ...
    vkGetPhysicalDeviceProperties2KHR(m_vkPhysicalDevice, &vkPhysicalDeviceProperties2KHR);

    memcpy(m_vkDeviceUUID, vkPhysicalDeviceIDPropertiesKHR.deviceUUID, sizeof(m_vkDeviceUUID));
    memcpy(m_vkDeviceLUID, vkPhysicalDeviceIDPropertiesKHR.deviceLUID, sizeof(m_vkDeviceLUID));
    m_vkDeviceNodeMask = vkPhysicalDeviceIDPropertiesKHR.deviceNodeMask;

    uint32_t queueFamilyCount = 0;
    vkGetPhysicalDeviceQueueFamilyProperties(m_vkPhysicalDevice, &queueFamilyCount, NULL);
...
VulkanPhysicalDevice::getQueueFamilyList() const
{
    return m_queueFamilyList;
}

const VulkanMemoryHeapList &
VulkanPhysicalDevice::getMemoryHeapList() const
{
    return m_memoryHeapList;
}

const VulkanMemoryTypeList &
VulkanPhysicalDevice::getMemoryTypeList() const
{
    return m_memoryTypeList;
}

const uint8_t *
VulkanPhysicalDevice::getUUID() const
{
    return m_vkDeviceUUID;
}

const uint8_t *
VulkanPhysicalDevice::getLUID() const
{
    return m_vkDeviceLUID;
}

uint32_t
VulkanPhysicalDevice::getNodeMask() const
{
...
bool
operator<(
    const VulkanQueueFamily & queueFamilyA,
...
VulkanMemoryHeap::VulkanMemoryHeap(
    ...
    : m_memoryHeapIndex(memoryHeap.m_memoryHeapIndex),
      m_size(memoryHeap.m_size),
      m_memoryHeapFlag(memoryHeap.m_memoryHeapFlag)
{
...
VulkanMemoryHeap::VulkanMemoryHeap(
    ...
    : m_memoryHeapIndex(memoryHeapIndex),
      m_size(size),
...
uint64_t
VulkanMemoryHeap::getSize() const
{
    return m_size;
}
...
VulkanMemoryHeap::getMemoryHeapFlag() const
{
    return m_memoryHeapFlag;
}

VulkanMemoryHeap::operator uint32_t() const
{
    return m_memoryHeapIndex;
}
...
VulkanMemoryType::VulkanMemoryType(
    ...
    : m_memoryTypeIndex(memoryTypeIndex),
      m_memoryTypeProperty(memoryTypeProperty),
...
VulkanMemoryTypeProperty
VulkanMemoryType::getMemoryTypeProperty() const
{
    return m_memoryTypeProperty;
}

const VulkanMemoryHeap &
VulkanMemoryType::getMemoryHeap() const
{
    return m_memoryHeap;
}

VulkanMemoryType::operator uint32_t() const
{
...
uint32_t
VulkanQueueFamily::getQueueFlags() const
{
    return m_vkQueueFamilyProperties.queueFlags & (uint32_t)VULKAN_QUEUE_FLAG_MASK_ALL;
}

uint32_t
VulkanQueueFamily::getQueueCount() const
{
    return m_vkQueueFamilyProperties.queueCount;
}

VulkanQueueFamily::operator uint32_t() const
{
...
#if defined(_WIN32) || defined(_WIN64)
    enabledExtensionNameList.push_back(VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME);
    enabledExtensionNameList.push_back(VK_KHR_EXTERNAL_SEMAPHORE_WIN32_EXTENSION_NAME);
#else
    enabledExtensionNameList.push_back(VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME);
    enabledExtensionNameList.push_back(VK_KHR_EXTERNAL_SEMAPHORE_FD_EXTENSION_NAME);
#endif
...
VulkanDevice::getPhysicalDevice() const
{
...
void
VulkanQueue::submit(
    const VulkanSemaphoreList & waitSemaphoreList,
...
VulkanDescriptorSetLayoutBinding::VulkanDescriptorSetLayoutBinding(
    ...
    : m_vkDescriptorSetLayoutBinding(descriptorSetLayoutBinding.m_vkDescriptorSetLayoutBinding)
{
...
void
VulkanDescriptorSetLayout::VulkanDescriptorSetLayoutCommon(
...
void
VulkanPipelineLayout::VulkanPipelineLayoutCommon(
...
VulkanComputePipeline::getPipelineBindPoint() const
{
    ...
    if (descriptorSetLayoutBindingList.size())
    {
        ...
        for (size_t dslbIdx = 0; dslbIdx < descriptorSetLayoutBindingList.size(); dslbIdx++)
        {
            ...
            if (vkDescriptorTypeToDescriptorCountMap.find(vkDescriptorSetLayoutBinding.descriptorType) == vkDescriptorTypeToDescriptorCountMap.end())
            {
                ...
                vkDescriptorTypeToDescriptorCountMap[vkDescriptorSetLayoutBinding.descriptorType]++;
            }
        }
...
uint32_t
VulkanOffset3D::getX() const
{
    return m_vkOffset3D.x;
}

uint32_t
VulkanOffset3D::getY() const
{
    return m_vkOffset3D.y;
}

uint32_t
VulkanOffset3D::getZ() const
{
    return m_vkOffset3D.z;
}

VulkanOffset3D::operator VkOffset3D() const
{
    return m_vkOffset3D;
}
...
uint32_t
VulkanExtent3D::getWidth() const
{
    return m_vkExtent3D.width;
}

uint32_t
VulkanExtent3D::getHeight() const
{
    return m_vkExtent3D.height;
}

uint32_t
VulkanExtent3D::getDepth() const
{
    return m_vkExtent3D.depth;
}

VulkanExtent3D::operator VkExtent3D() const
{
    return m_vkExtent3D;
}
...
VulkanCommandBuffer::bindPipeline(
    const VulkanPipeline & pipeline)
{
    VkPipelineBindPoint vkPipelineBindPoint = (VkPipelineBindPoint)pipeline.getPipelineBindPoint();
    ...
    const VulkanDescriptorSet & descriptorSet)
{
    ...
    vkCmdBindDescriptorSets(m_vkCommandBuffer, vkPipelineBindPoint, pipelineLayout, 0, 1, &vkDescriptorSet, 0, NULL);
}
    ...
    VulkanImageLayout newImageLayout)
{
    ...
    vkCmdPipelineBarrier(m_vkCommandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, NULL, 0, NULL, (uint32_t)vkImageMemoryBarrierList.size(), vkImageMemoryBarrierList.data());
}
...
void
VulkanCommandBuffer::fillBuffer(
    const VulkanBuffer & buffer,
    uint32_t data,
    uint64_t offset,
    uint64_t size)
...
void
VulkanCommandBuffer::copyBufferToImage(
    const VulkanBuffer & buffer,
    ...
    VulkanImageLayout imageLayout)
{
    ...
    for (uint32_t mipLevel = 0; mipLevel < image.getNumMipLevels(); mipLevel++)
    {
        ...
        bufferOffset += extent3D.getWidth() * extent3D.getHeight() * extent3D.getDepth() * elementSize;
        bufferOffset = ROUND_UP(bufferOffset, std::max(elementSize, (size_t)VULKAN_MIN_BUFFER_OFFSET_COPY_ALIGNMENT));
    }

    vkCmdCopyBufferToImage(m_vkCommandBuffer, buffer, image, (VkImageLayout)imageLayout, (uint32_t)vkBufferImageCopyList.size(), vkBufferImageCopyList.data());
}
...
void
VulkanCommandBuffer::copyImageToBuffer(
    const VulkanImage & image,
    const VulkanBuffer & buffer,
    uint64_t bufferOffset,
    uint32_t mipLevel,
...
VulkanBuffer::~VulkanBuffer()
{
    vkDestroyBuffer(m_device, m_vkBuffer, NULL);
}

uint64_t
VulkanBuffer::getSize() const
{
    return m_size;
}

uint64_t
VulkanBuffer::getAlignment() const
{
    return m_alignment;
}

const VulkanMemoryTypeList &
VulkanBuffer::getMemoryTypeList() const
{
    return m_memoryTypeList;
}
...
VulkanImage::VulkanImage(
    ...
    : m_device(image.m_device),
      ...
      m_extent3D(image.m_extent3D),
      m_format(image.m_format),
      m_numMipLevels(image.m_numMipLevels),
      m_numLayers(image.m_numLayers),
      m_vkImage(image.m_vkImage),
      m_size(image.m_size),
      m_alignment(image.m_alignment),
...
VulkanImage::VulkanImage(
    ...
    VulkanExternalMemoryHandleType externalMemoryHandleType,
    VulkanImageCreateFlag imageCreateFlag,
...
VulkanImage::~VulkanImage()
{
    vkDestroyImage(m_device, m_vkImage, NULL);
}

VulkanExtent3D
VulkanImage::getExtent3D(
...
uint32_t
VulkanImage::getNumMipLevels() const
{
    return m_numMipLevels;
}

uint32_t
VulkanImage::getNumLayers() const
{
    return m_numLayers;
}
...
uint64_t
VulkanImage::getAlignment() const
{
    return m_alignment;
}

const VulkanMemoryTypeList &
VulkanImage::getMemoryTypeList() const
{
    return m_memoryTypeList;
}
...
VulkanImage2D::VulkanImage2D(
    ...
    VulkanImageUsage imageUsage,
    VulkanSharingMode sharingMode)
    : VulkanImage(device, VULKAN_IMAGE_TYPE_2D, format, VulkanExtent3D(width, height, 1), numMipLevels, 1, externalMemoryHandleType, imageCreateFlag, VULKAN_IMAGE_TILING_OPTIMAL, imageUsage, sharingMode)
{
...
VulkanImageView::VulkanImageView(
    const VulkanImageView & imageView)
    : m_device(imageView.m_device),
      m_vkImageView(imageView.m_vkImageView)
{
...
VulkanDeviceMemory::VulkanDeviceMemory(
    ...
    : m_device(deviceMemory.m_device),
      ...
      m_size(deviceMemory.m_size),
      m_isDedicated(deviceMemory.m_isDedicated)
{
...
uint64_t
VulkanDeviceMemory::getSize() const
{
    return m_size;
}
...
bool
VulkanDeviceMemory::isDedicated() const
{
    return m_isDedicated;
}

void *
VulkanDeviceMemory::map(
    size_t offset,
    size_t size)
{
    void *pData;

    vkMapMemory(m_device, m_vkDeviceMemory, (VkDeviceSize)offset, (VkDeviceSize)size, 0, &pData);
...
VulkanDeviceMemory::operator VkDeviceMemory() const
{
...
VulkanSemaphore::VulkanSemaphore(
    ...
    : m_device(semaphore.m_device),
      m_vkSemaphore(semaphore.m_vkSemaphore)
{
      ...
      m_name(name)
{
    ...
    vkSemaphoreCreateInfo.pNext = (externalSemaphoreHandleType != VULKAN_EXTERNAL_SEMAPHORE_HANDLE_TYPE_NONE) ? &vkExportSemaphoreCreateInfoKHR : NULL;
...
#if defined(_WIN32) || defined(_WIN64)
HANDLE VulkanSemaphore::getHandle(VulkanExternalSemaphoreHandleType externalSemaphoreHandleType) const
{        
        HANDLE handle;
