...
const char* wg_scan_local_work_group_size =
    "__kernel void test_wg_scan_local_work_group_size(global size_t *output)\n"
    "{\n"
    "\n"
    "    if(get_global_linear_id() != 0) return;"
    "    for (uint i = 0; i < 3; i++)\n"
    "    {"
    "         output[i] = get_enqueued_local_size(i);\n"
    "    }\n"
    "}\n"
    ""
    "__kernel void test_wg_scan_local_work_group_size_static_local(global "
    "size_t "
    "*output)\n"
    "{\n"
    ...
    "}\n"
    ""
    "__kernel void test_wg_scan_local_work_group_size_dynlocal(global size_t "
    "*output, __local char * c)\n"
    "{\n"
    "    if(get_global_linear_id() != 0) return;"
    "    for (uint i = 0; i < 3; i++)\n"
    "    {"
