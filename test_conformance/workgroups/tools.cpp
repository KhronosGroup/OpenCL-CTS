...
void PrimeNumbers::generatePrimeNumbers(unsigned int maxValue)
{
    ...
    for (unsigned int i = 2; i < maxValue; i++) primeNumbers.push_back(i);
...
{
    if (lowerValue >= higherValue) return -1;

    if (primeNumbers.back() < lowerValue) return -2;
...
PrimeNumbers::Result2d PrimeNumbers::fitMaxPrime2d(size_t val1, size_t val2,
                                                   size_t productMax)
{
...
PrimeNumbers::Result3d PrimeNumbers::fitMaxPrime3d(size_t val1, size_t val2,
                                                   size_t val3,
                                                   size_t productMax)
{
...
ErrorMap::value_type rawDataErrorString[] = {
    ...
    ErrorMap::value_type(
        ERR_STRICT_MODE,
        "strict requirements failed. Wrong local work group size"),
...
ErrorClass::ErrorClass()
{
    ...
    for (unsigned short i = 0;
         i < sizeof(_errorArrayCounter) / sizeof(_errorArrayCounter[0]); i++)
    {
...
void ErrorClass::show(Type err, std::string where, std::string additionalInfo)
{
    ...
    if (_overallNumberOfErrors == MAX_NUMBER_OF_PRINTED_ERRORS)
        printError(
            "\t. . . Too many errors. Application will skip printing them.");

    if (_overallNumberOfErrors >= MAX_NUMBER_OF_PRINTED_ERRORS) return;
...
void ErrorClass::show(Type whatErr, std::string where, cl_ulong valueIs,
                      cl_ulong valueExpected)
{
...
void ErrorClass::showStats()
{
    ...
    {
        err = (errorString.find(it->first) == errorString.end()) ? ERR_UNKNOWN
                                                                 : it->first;
...
bool ErrorClass::checkError() { return _overallNumberOfErrors > 0; }

// This method is required to synchronize errors counters between kernel and
// host
void ErrorClass::synchronizeStatsMap()
{
    for (unsigned short i = 0;
         i < sizeof(_errorArrayCounter) / sizeof(_errorArrayCounter[0]); i++)
    {
        if (_errorArrayCounter[i] == 0) continue;
