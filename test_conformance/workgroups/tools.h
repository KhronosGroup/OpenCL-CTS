...
public:
    ...
    // Return val1 and Val2 which are largest prime numbers who's product is <=
    // productMax
    static Result2d fitMaxPrime2d(size_t Val1, size_t Val2, size_t productMax);
    // Return val1, val2 and val3, which are largest prime numbers who's product
    // is <= productMax
    static Result3d fitMaxPrime3d(size_t Val1, size_t Val2, size_t Val3,
                                  size_t productMax);
...
public:
    ErrorClass();
    void show(Type whatErr, std::string where = "",
              std::string additionalInfo = "");
    void show(Type whatErr, std::string where, cl_ulong valueIs,
...
private:
    cl_uint _errorArrayCounter[Error::_LAST_ELEM]; // this buffer is passed to
                                                   // kernel
